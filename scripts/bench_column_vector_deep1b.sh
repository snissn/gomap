...
if [[ "${RUN_NEIGHBORHOOD_SMOKE}" == "true" || "${RUN_NEIGHBORHOOD_SMOKE}" == "1" ]]; then
  ...
  GOWORK=off go test ./experiments/colgranule \
    -run '^$' \
    -bench '^BenchmarkColumnVectorGraphDeep1B(JZIPNeighborhoodCompressionSmoke|JZIPDecodeAndScoreSmoke|SphericalDirectScore|LocalFrameApproxScore|GranuleNativeScore|GranuleNativeMicroKernels|NeighborhoodCompressionSmoke)/' \
    -benchmem \
    -benchtime "${BENCHTIME}" \
    -count "${COUNT}" \
    -timeout "${GO_TEST_TIMEOUT}"
