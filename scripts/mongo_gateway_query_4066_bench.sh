...
done
{
  ...
  awk '/BenchmarkMongoNegativeDottedQueryShapes\// {n=$1; sub(/^.*\//,"",n); ns=$3; b=$7; al=$9; sub("ns/op","",ns); sub("B/op","",b); sub("allocs/op","",al); c[n]++; a[n,c[n]]=ns; if(!(n in lo)||b<lo[n])lo[n]=b;if(b>hi[n])hi[n]=b;if(!(n in alo)||al<alo[n])alo[n]=al;if(al>ahi[n])ahi[n]=al} END {for(n in c){for(i=2;i<=c[n];i++)for(j=i;j>1&&a[n,j]<a[n,j-1];j--){t=a[n,j];a[n,j]=a[n,j-1];a[n,j-1]=t}; p50=a[n,int((50*c[n]+99)/100)]; p95=a[n,int((95*c[n]+99)/100)]; p99=a[n,int((99*c[n]+99)/100)]; printf "- %s: p50=%sns/op p95=%sns/op p99=%sns/op ops/s=%.1f B/op=%s-%s allocs/op=%s-%s\n",n,p50,p95,p99,1e9/p50,lo[n],hi[n],alo[n],ahi[n]}}' "$out/raw.txt"
} > "$out/report.md"
cat "$out/report.md"
