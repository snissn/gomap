...
func runVlogPostsyncOptimize(dir string, args []string) {
	...
	strategy := fs.String("strategy", "offline", "Optimization strategy: offline|explicit|hybrid")
	...
	if mode == "offline" || mode == "hybrid" {
		...
		report.Offline = &valueLogPostsyncOfflineReport{
	...
	fmt.Printf("dir=%s strategy=%s initial_bytes=%d final_bytes=%d total_ms=%d\n", report.Dir, report.Strategy, report.InitialBytes, report.FinalBytes, report.TotalMillis)
...
func parseValueLogPostsyncStrategy(s string) (string, error) {
	...
	case "hybrid":
		return "hybrid", nil
	default:
		return "", fmt.Errorf("unsupported -strategy=%q (expected offline|explicit|hybrid)", s)
	}
}
