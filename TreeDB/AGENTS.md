...
- 2026-01-22 PR-AT7: fixed autotune bench warmup for incompressible workloads; kept probe keep-policy active; added metric seeding and early CPU-bound skip in writer; adjusted skip backoff; bench: `go run ./TreeDB/cmd/unified_bench -suite vlog_autotune -validate`.
- 2026-01-22 PR-AT8: added treemap scan-jsonl output with binary-safe encoding + encoding field; vlog_dict_realdata now decodes encoded JSONL via `encoding` or `-input-encoding`; tests: not run.
- 2026-01-22 PR-AT9: added treemap import-jsonl + JSONL roundtrip tests (string/base64/hex), invalid base64 failure test, and truncated JSONL mismatch; test attempt: `go test ./cmd/treemap -count=1` failed due to Go toolchain mismatch (go1.25.5 vs 1.25.4).
