...
// Optional key-aware pointer reads for value-log payloads.
...
// GetEntry returns the exact persisted leaf entry for key.
//
...
func (t *Tree) GetAppend(key, dst []byte) ([]byte, error) {
	val, ptr, flags, _, err := t.lookupLeafValueView(key, dst, true)
	if err != nil {
		return dst, err
	}
	if flags&node.FlagTombstone != 0 {
