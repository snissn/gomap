...
func TestConsistencyStress(t *testing.T) {
	...
	// Error channel to safely propagate errors from goroutines to main test thread.
	// Buffered to avoid blocking goroutines when multiple errors occur.
	...
	go func() {
		...
		for h := int64(1); h <= maxHeight; h++ {
			...
			if err := cdb.Set(key, val); err != nil {
				// Send error to channel instead of calling t.Errorf from goroutine
				select {
				case errChan <- fmt.Errorf("Set validator failed: %v", err):
				default:
				}
				return
			}
			...
			if h%100 == 0 {
				...
				if err := b.Write(); err != nil {
					// Send error to channel instead of calling t.Errorf from goroutine
					select {
					case errChan <- fmt.Errorf("Bypass write failed: %v", err):
					default:
					}
				}
			}
			...
			if target > lastVerified {
				for h := lastVerified + 1; h <= target; h++ {
					...
					if err != nil {
						// Send error to channel instead of calling t.Errorf from goroutine
						select {
						case errChan <- fmt.Errorf("Get failed for %s: %v", key, err):
						default:
						}
						return
					}
					if val == nil {
						// Send critical error to channel instead of calling t.Fatalf from goroutine
						// This was the line 127 error caught by go vet
						select {
						case errChan <- fmt.Errorf("CRITICAL: Key %s missing!", key):
						default:
						}
						return
					}
					if len(val) != 2048 {
						// Send error to channel instead of calling t.Errorf from goroutine
						select {
						case errChan <- fmt.Errorf("Key %s corrupted len %d", key, len(val)):
						default:
						}
					}
					...
					if readH != uint64(h) {
						// Send error to channel instead of calling t.Errorf from goroutine
						select {
						case errChan <- fmt.Errorf("Key %s content mismatch: got height %d", key, readH):
						default:
						}
					}
				}
	...
	// This ensures all errors are reported in the main test goroutine
	close(errChan)
	var errors []error
	...
	if len(errors) > 0 {
		...
		// Use t.Fatalf for critical errors (those that were originally t.Fatalf)
		for _, err := range errors {
			if strings.Contains(err.Error(), "CRITICAL") {
				t.Fatalf("Test failed with critical error")
			}
		}
	}
}
