...
| Command | SCRAM-SHA-256 authentication / built-in authorization | `supported subset` | `TestSCRAMSHA256EstablishesConnectionIdentityAndGatesCommands`, `TestAuthorizationEverySupportedCommandHasExplicitPrivilege`, role/cursor/admin/reopen tests, official-driver SCRAM and authorization tests | Durable scoped `read`, `readWrite`, `dbAdmin`, `userAdmin`, and `serverAdmin` grants authorize supported standalone commands before protected observation or mutation. Lists are filtered, cursors are principal-bound, the last enabled server administrator is protected, and unbound routed access fails closed. No external identity mapping or distributed role catalog is claimed. |
