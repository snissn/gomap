...
| `$ne`, `$nin`, `$exists` | `supported subset` | `TestMongoNegativeAndExistsPredicatesRespectMissingNullAndDottedValues`, `TestMongoNegativePredicateRejectsOverCapBeforeCursorOrMutation` | Negative/existence predicates remain residual. `$ne:null` requires a non-null existing value; other `$ne` and `$nin` include missing. `$nin` choices and boolean branch/predicate work have explicit caps. |
