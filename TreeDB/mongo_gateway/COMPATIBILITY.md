...
| Command | `update` / `updateOne` helper path | `supported subset` | `TestMongoCompatibilityMatrix`, update tests, `TestMongoFirstWriteConcurrentUpdateUpserts`, `TestMongoFirstWriteLateExistingMutationsWait`, `TestMongoFirstWriteStalePendingNamespaceDoesNotWait` | Only `_id`-targeted updateOne with accepted update shapes. Competing in-process first-write upserts serialize collection creation and its first mutation after an initial miss; existing-collection duplicate handling is unchanged. |
