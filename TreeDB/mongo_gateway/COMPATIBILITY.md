...
| Command | `findAndModify` | `supported subset` | `TestFindAndModifyReturnsAtomicBeforeAndAfterImages`, `TestMongoCompatibilityMatrix` | Exact `_id` query with top-level `$set`/`$inc`/`$unset` or replacement; pre-image by default, `new:true` post-image, optional top-level `fields`, and upsert. A same-`_id` upsert losing the initial insert retries the existing-document mutation only for `ErrDocumentExists`; other duplicate conflicts fail. Cluster/routed, remove, sort, dotted projection, and transaction/retry markers are rejected. |
