...
| Command | `findAndModify` | `supported subset` | `TestFindAndModifyReturnsAtomicBeforeAndAfterImages`, `TestFindAndModifyInsertConflictAppliesToExistingDocument`, `TestMongoFirstWriteConcurrentFindAndModifyUpserts`, `TestStandaloneServerOfficialDriverConcurrentFirstWriteFindAndModifyUpserts`, `TestStandaloneServerOfficialGoDriverFilterWrites`, `TestMongoNegativePredicatesShareReadAndFilterWriteConsumers`, `TestMongoCompatibilityMatrix` | Exact `_id` retains its direct path; supported non-`_id` filters select natural order and recheck at mutation. Replacement and the shared dotted `$set`/`$unset`/`$inc`, `$setOnInsert`, bounded `$push`, and bounded `$addToSet` modifier subset are available; pre-image by default, `new:true` post-image, optional document-only dotted `fields`, and exact-`_id` upsert. `$setOnInsert` applies only to insertion. Competing in-process first-write upserts serialize collection creation and its first mutation after an initial miss. A same-`_id` upsert losing the initial insert retries as an update only for `ErrDocumentExists`, returning `updatedExisting: true` without `upserted`; other duplicate conflicts fail. Cluster/routed, remove, sort, positional/array-filter paths, and transaction/retry markers are rejected. |
...
| `$ne`, `$nin`, `$exists` | `supported subset` | `TestMongoNegativeAndExistsPredicatesRespectMissingNullAndDottedValues`, `TestMongoNegativeQueryWorkCapsRejectBeforeCursorOrMutation` | Negative/existence predicates remain residual. `$ne:null` requires a non-null existing value; other `$ne` and `$nin` include missing. `$nin` choices and boolean branch/predicate work have explicit caps. |
| `$nor`, field `$not` | `supported subset` | `TestMongoTopLevelNorCombinesWithSiblingPredicates`, `TestMongoNegativeAndExistsPredicatesRespectMissingNullAndDottedValues`, `TestMongoNegativePredicateRejectsNestedAndDuplicateOperators` | Top-level `$nor` and single-inner-predicate field `$not` are bounded and residual. Nested `$not`, array traversal, malformed paths, duplicate operators, and over-cap boolean work reject before observation or mutation. |
| Regex/text/geospatial predicates | `not implemented` | Unsupported operators reject or command is absent | Out of MVP scope. |
| Dotted predicates into nested documents | `supported subset` | dotted predicate and array-boundary tests | Negative/existence paths, projection, and sort traverse nested documents only; array traversal rejects. No multikey query or index compatibility claim. |
...
| Projection include/exclude fields | `supported subset` | `TestMongoCompatibilityMatrix`, `TestMongoDottedProjectionRebuildsParentsAndFailsClosedOnArrays` | Cannot mix include/exclude except `_id`; dotted inclusion reconstructs requested document parents and nested `_id` remains ordinary. Dotted array traversal and malformed/over-depth paths reject. |
| Document-field sort | `supported subset` | `TestMongoCompatibilityMatrix`, `TestMongoDottedSortIsStableAndReportsBoundedMaterialization`, compound planner tests | One or more top-level or document-only dotted terms are accepted. A compatible one-term BSON-v2 compound index order (or its complete reverse) streams with stable `_id` ties. Multi-term and all other supported sorts use bounded in-memory Mongo sorting before pagination, because missing/null values can occupy distinct physical index runs. Dotted array traversal rejects. |
