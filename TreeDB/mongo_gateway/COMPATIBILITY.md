...
| Nested update modifiers | `supported subset` | `TestMongoMutationNestedOperators`, `TestMongoMutationSetOnInsertOnlyAppliesToInsertion`, `TestStandaloneServerOfficialGoDriverFilterWrites`, `TestMongoMutationCommandWALValueLogPointersReopen`, `TestMongoCompatibilityMatrix` | Dotted `$set`, `$unset`, `$inc`, `$setOnInsert`, scalar or at-most-256-item `$each` `$push`, and BSON-equality `$addToSet` are shared by exact-ID and supported filter writes. An update targets at most 256 fields; each dotted path has at most 100 components; `$addToSet` duplicate work is capped at 65,536 comparisons and 8 MiB of worst-case BSON value bytes per mutation. `$setOnInsert` applies only when an exact-ID upsert inserts; empty operator specifications and empty `$each` are no-ops. `_id`, numeric array-index components, positional/array-filter paths, empty path segments, ancestor/descendant conflicts, pipelines, and unsupported modifiers reject before mutation. `$inc` supports int32/int64/double only; null/non-numeric targets reject. |
