...
| Top-level sort | `supported subset` | `TestMongoCompatibilityMatrix`, compound planner tests | One or more top-level terms are accepted. Compatible BSON-v2 compound index order (or its complete reverse) streams with stable `_id` ties; all other supported sorts use bounded in-memory sorting. Dotted sort is rejected. |
