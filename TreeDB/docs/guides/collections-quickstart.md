...
| --- | --- | --- |
| `column-store writes require command WAL` | The DB format config did not require command-WAL v1. | For current typed-storage examples, call `backenddb.SaveFormatConfig(dir, backenddb.FormatConfig{RequiredFeatures: []string{backenddb.RequiredFeatureCommandWALV1}})` before opening a new DB directory. |
