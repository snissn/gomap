...
| Dense fixed-degree `adjacency_list` typed-column sections | Landed durable publication/reconstruction for non-nullable owners with positive `adjacency_degree`; direct-view scan benchmark covers row-major `uint32` sections. | `BenchmarkTypedColumnAdjacencyDenseDirectViewScan`. |
...
| Vector adjacency query switching | Authoritative fixed-degree `adjacency_list` publication is available, but vector graph/search routing to that owner remains follow-up work. | [#1782](https://github.com/snissn/gomap/issues/1782) |
