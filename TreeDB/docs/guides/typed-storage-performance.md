...
| Dense fixed-degree `adjacency_list` typed-column sections | Landed durable publication/reconstruction for non-nullable owners with positive `adjacency_degree`; direct-view certification is deferred/fallback-only for the active #1886 stack (#1901). | `BenchmarkTypedColumnAdjacencyDenseFallbackScan`. |
...
| Vector adjacency query switching | Authoritative fixed-degree `adjacency_list` publication is available, but certified adjacency mmap direct views and graph/search direct-view routing are deferred. | [#1782](https://github.com/snissn/gomap/issues/1782), [#1901](https://github.com/snissn/gomap/issues/1901) |
