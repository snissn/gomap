...
| Adjacency list | current quarantined `column_graph` adjacency direct sources; target is typed-column `uint32_list` vector-index state | The current `adjacency_layout: "uint32_offsets_list"` path publishes graph-specific variable-adjacency sources that native search can consume, but #1983 marks this as transitional compatibility. Future work should use generic `uint32_list` assets owned by vector-index state; row-asset/dense paths remain fallback/compatibility until then. |
...
| Certified all-layer `column_graph` adjacency direct sources are current transitional compatibility only; #1983 quarantines them so #1984/#1985 can introduce generic `uint32_list` storage and #1986/#1988 can layer vector-index state/search above it. Row-asset adjacency remains a legacy/corruption fallback, and dense fixed-degree compatibility remains a separate fallback path. | [#1983](https://github.com/snissn/gomap/issues/1983), [#1984](https://github.com/snissn/gomap/issues/1984), [#1985](https://github.com/snissn/gomap/issues/1985), [#1986](https://github.com/snissn/gomap/issues/1986), [#1988](https://github.com/snissn/gomap/issues/1988) |
...
| Adjacency direct-source counters show scratch decodes | The current quarantined `column_graph` adjacency direct sources are missing, stale, or failed validation, so search fell back to row-asset adjacency or dense compatibility/corruption recovery. | For current assets, rebuild or republish the graph so every layer has a certified `uint32_offsets_list` adjacency source; for new architecture work, prefer the #1984/#1985/#1986 `uint32_list` vector-index state path. |
