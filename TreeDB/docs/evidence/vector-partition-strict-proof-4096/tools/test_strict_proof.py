...
class StrictProofRunnerTest(unittest.TestCase):
    def test_result_requires_one_ingress_proof_and_no_request_opens(self) -> None:
        def stage(reads: int) -> dict[str, int]:
            return {"reads": reads, "no_log_proofs": reads, "log_barriers": 0}
        ...
        def cell(concurrency: int) -> dict[str, object]:
            return {
                ...
                "catalog_reads": {"total": {"total": stage(1000), "strict_search": stage(1000), "operations_health": stage(0), "coordinator_lifecycle": stage(0), "shard_lifecycle": stage(0), "unknown": stage(0)}},
            }
