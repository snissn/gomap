#!/usr/bin/env python3
"""Run one serialized #4019 M3 TreeDB four-container row."""
...
import run_m3_treedb as common
...
def run_one(corpus_id: str, repetition: int) -> None:
    ...
    run = common.ROOT / "verified-runs" / topology / corpus_id / f"repeat-{repetition}"
    run.mkdir(parents=True)
    client = run / "client"
    client.mkdir()
    network = f"gomap4019-m3-{corpus_id}-r{repetition}"
    ...
    configs = [common.node_config(run, topology, corpus, index, endpoints, f"0.0.0.0:{PUBLIC_PORT}", applied) for index in range(4)]
    ...
    try:
        ...
        command = ["docker", "exec", names[0], *inner]
    ...
    resources.update({
        ...
        "network": network, "persistent_bytes": persistent,
    ...
    print(f"complete topology={topology} corpus={corpus_id} repetition={repetition} result={client / 'search.json'}", flush=True)


def main() -> None:
    parser = argparse.ArgumentParser()
    parser.add_argument("corpus", choices=tuple(common.CORPORA))
    parser.add_argument("repetition", type=int, choices=(1, 2, 3))
    args = parser.parse_args()
    run_one(args.corpus, args.repetition)
