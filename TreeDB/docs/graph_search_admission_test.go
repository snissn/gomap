...
func TestDocs_GraphSearchTypedColumnAdmissionGate(t *testing.T) {
	...
	required := []struct {
		...
		{role: "Base vectors", owner: "typed_column_part", logical: "float32_vector", encoding: "raw_float32_vector", tier: "mmap_direct", status: "pending", shape: "[]float32", boundary: "Candidate scoring", fallback: "MUST NOT fetch candidate vectors", counter: "vector_mmap_direct/search", test: "dimension mismatch", benchmark: "BenchmarkColumnVectorGraphNativeSearchCosineV3"},
		{role: "HNSW adjacency", owner: "adjacency", logical: "uint32_list", encoding: "raw_uint32_offsets_list", tier: "mmap_direct", status: "pending", shape: "CSR", boundary: "Edge traversal", fallback: "MUST NOT read legacy graph-row adjacency", counter: "adjacency_typed_list_mmap_direct/search", test: "missing layer", benchmark: "edges/search"},
		{role: "Inverse norms", owner: "inverse_norm", logical: "float32", encoding: "raw_float32", tier: "mmap_direct", status: "pending", shape: "[]float32", boundary: "Candidate scoring", fallback: "MUST NOT read inverse norms from legacy graph rows", counter: "norm_mmap_direct/search", test: "finite-value", benchmark: "graph-only search"},
		{role: "Row refs", owner: "row_refs", logical: "int64", encoding: "raw_int64", tier: "mmap_direct", status: "pending", shape: "row-ref arrays", boundary: "Result and document-fetch", fallback: "MUST NOT scan legacy graph row IDs", counter: "row_ref_state_result_refs", test: "coordinate bounds", benchmark: "doc-fetch timing"},
		{role: "Document IDs", owner: "document_ids", logical: "bytes", encoding: "raw_bytes_offsets", tier: "mmap_direct", status: "pending", shape: "Offsets []uint64", boundary: "Final top-k", fallback: "MUST NOT return IDs from legacy graph rows", counter: "result_id_typed_bytes_state", test: "arbitrary binary IDs", benchmark: "BenchmarkSearchVectorIndexColumnGraphNativeReaderWithDocumentsV4"},
	...
	for _, contract := range contracts {
		...
		case "normalized_vectors":
			if !graphSearchAdmissionCellHasStatus(cells[4], "deferred") && !graphSearchAdmissionCellHasStatus(cells[4], "experimental") {
				t.Fatalf("normalized_vectors admission status=%q, want deferred or experimental", cells[4])
			}
		default:
