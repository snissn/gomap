...
continues to use the streaming decode path. `uint64` is a direct payload only for
stats/pruning purposes until a native uint64 payload exists. Raw int64 carriers
for scalar floats fail closed at both layers: they preserve bits for
