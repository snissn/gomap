...
- A generic `packed_uint_vector` physical form may represent the three packed
  variants with `bits_per_element in {1,2,4}`.
- `uint64_word_vector` is a compatible access/view mode over row bytes, not a
  separate sidecar format.

#1931 must not overload #1929 scalar `uint8` or `uint64` columns for row-major
packed payloads, and it must not replace ordinary dense numeric vectors owned by
#1930.
...
| Field | Meaning |
| --- | --- |
| `logical_elements_per_row` | Number of values/dimensions in each row. |
| `bits_per_element` | `1`, `2`, or `4`. |
| `logical_bits_per_row` | `logical_elements_per_row * bits_per_element`. |
| `physical_bytes_per_row` | `ceil(logical_bits_per_row / 8)`. |
| `physical_words_per_row` | `ceil(physical_bytes_per_row / 8)` for scratch word access. |
...
padding and must be zero. Writers must zero them. Read/open validation under
integrity modes must fail closed on non-zero padding, section-length mismatch,
row-width mismatch, unsupported `bits_per_element`, or inconsistent
logical/physical metadata.

When `physical_bytes_per_row` is not a multiple of 8, scratch word access
zero-fills bytes after the physical row in the final partial word. This word
padding is an access rule; it is not additional on-disk payload.
...
`TreeDB/internal/typedcolumn/packed_code.go` provides helper-level contracts for
this issue without allocating persistent type IDs:
...
Broad adapter, descriptor, layout-contract, manifest, reopen, and direct-view
certification integration remains blocked on final #1929/#1930 contracts.
