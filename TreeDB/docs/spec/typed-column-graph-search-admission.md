...
| HNSW search pack | Vector-index state role `hnsw_search_pack` with asset id `hnsw_search_pack_v1`; current rebuilds publish the durable pack reference, but reader/search-route promotion remains deferred. | `hnsw_search_pack` / `hnsw_search_pack_v1` | `mmap_direct` when admitted | `deferred` after #2313 writer publication | Future single pack view over normalized vectors, levels, adjacency CSR, row refs, and document IDs, all bound to one base manifest identity. | Not in the current healthy loop; future pack routing may use it for candidate scoring, edge traversal, and final top-k identity only after reader and search-route promotion. | Healthy current search must ignore this role or fail closed during validation until a later PR explicitly promotes it; it MUST NOT route production search through the pack in the writer-only PR. | Format and writer tests cover encode/decode, truncation, bad magic/version, corrupt directory, length, row/dim/stride/alignment, adjacency ordinal, base identity mismatch, rebuild publication, reopen identity validation, reachability/accounting, rewrite remap, and fallback-route usability. Route counters already reserved include `hnsw_search_pack_active/search`, `hnsw_search_pack_invalid/search`, and `hnsw_search_pack_fallbacks/search`. | #2312 records synthetic pack decode validation timing; #2313 records rebuild/storage and no-route-regression evidence. Future reader/search-route promotion must add open/search benchmarks with `ns/op`, `ops/sec`, `B/op`, `allocs/op`, pack-active/no-fallback counters, memory residency, and direct/fallback evidence before changing status. |
