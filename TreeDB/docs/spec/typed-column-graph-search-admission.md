...
| Quantized scalar_u8 codes | Vector-index state role `quantized_codes` with asset id `quantized/<name>/codes` for each declared quantized score plane. | `byte_vector` / `raw_fixed_bytes` | `mmap_direct` | `pending` | Prepared `quantizedasset` codes view with one fixed-byte row per graph ordinal and immutable base-graph identity. | Future quantized scorer only; exact/default traversal and scoring must not touch it. | Quantized modes must fail closed when this role is missing, stale, corrupt, below `mmap_direct`, mismatched to the base graph, or when the scorer is unavailable; they MUST NOT silently return exact scores. | Tests cover rebuild, reopen, `quantizedasset.Prepare` validation, missing/stale/mismatch failure, and exact-mode unchanged. Future promotion must add role counters for quantized code direct/prepared/fallback access. | #1926 asset-lifecycle evidence reports storage bytes/vector and test coverage. Future scorer promotion must update this row with exact vs quantized/rerank `ns/op`, `ops/sec`, `B/op`, `allocs/op`, recall@K, and direct/fallback counters. |
