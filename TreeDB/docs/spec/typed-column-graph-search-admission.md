...
| Quantized scalar_u8 codes | Vector-index state role `quantized_codes` with asset id `quantized/<name>/codes` for each declared quantized score plane. | `byte_vector` / `raw_fixed_bytes` | `mmap_direct` | `admitted` for explicit scalar_u8 quantized modes by #1926 closeout | Prepared `quantizedasset` codes view with one fixed-byte row per graph ordinal and immutable base-graph identity. | Explicit `quantized_only` traversal/final ranking and explicit `quantized_rerank` traversal/candidate collection only; exact/default traversal and scoring must not touch it. `quantized_rerank` traverses the normalized `ef_search` candidate pool, trims to `QuantizedRerankCandidates`, and exact-scores only the validated shortlist through the authoritative float32 vector/norm path. | Quantized modes must fail closed when this role is missing, stale, corrupt, below `mmap_direct`, mismatched to the base graph, or when the scorer is unavailable; they MUST NOT silently fall back to exact candidate collection. | Counters and tests cover rebuild, reopen, `quantizedasset.Prepare` validation, missing/stale/mismatch failure, exact-mode unchanged, quantized code score/read counters, no exact vector/norm scoring in `quantized_only`, exact final scores in `quantized_rerank`, normalized `ef_search` traversal before shortlist trim, and `quantized_rerank_exact_score_calls/search`. | #1926 closeout uses `BenchmarkColumnGraphScalarU8QuantizedScorePlanes1926` and `BenchmarkColumnGraphScalarU8QuantizedRebuildStorage1926` to report exact vs `quantized_only` vs `quantized_rerank` `ns/op`, `ops/sec`, `B/op`, `allocs/op`, recall@K, candidates/search, rerank candidates/search, quantized code bytes, exact vector/norm bytes, storage bytes/vector, rebuild cost, and zero hidden exact fallback counters; current scalar Go scorer evidence makes no speedup claim. |
