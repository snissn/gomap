...
| HNSW search pack | Vector-index state role `hnsw_search_pack` with asset id `hnsw_search_pack_v1`; durable pack reference only until the writer/reader/search-route issues promote it. | `hnsw_search_pack` / `hnsw_search_pack_v1` | `mmap_direct` when admitted | `deferred` by #2312 format-only contract | Future single pack view over normalized vectors, levels, adjacency CSR, row refs, and document IDs, all bound to one base manifest identity. | Not in the current healthy loop; future pack routing may use it for candidate scoring, edge traversal, and final top-k identity only after reader and search-route promotion. | Healthy current search must ignore this role or fail closed during validation until a later PR explicitly promotes it; it MUST NOT route production search through the pack in the format-only PR. | Format tests cover encode/decode, truncation, bad magic/version, corrupt directory, length, row/dim/stride/alignment, adjacency ordinal, and base identity mismatch failures; route counters already reserved include `hnsw_search_pack_active/search`, `hnsw_search_pack_invalid/search`, and `hnsw_search_pack_fallbacks/search`. | #2312 records synthetic pack decode validation timing only; future promotion must add open/search benchmarks with `ns/op`, `ops/sec`, `B/op`, `allocs/op`, pack-active/no-fallback counters, memory residency, and direct/fallback evidence before changing status. |
