...
then includes each later document mutation's live-carrier delta in that
mutation's ordered document/column/locator/system-root publication. Replay
never scans collection rows and never folds or rebuilds a `column_graph`.
...
coverage are independent clocks and are validated separately. A replayed
document command advances its durable `AppliedCommandLSN` only in the same
publication that advances the carrier's revision and exact collection
document-generation coverage. A process loss before handler reconciliation or
an `Open` finalizer therefore cannot strand durable documents behind stale
carrier coverage, and a following checkpoint needs no extra carrier command.
