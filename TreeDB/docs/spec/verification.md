...
| Collection-level Raft placement resolves each placed `{database,catalog,collection}` to exactly one group and fails closed for unplaced collections, duplicate placements, unknown groups/features, and invalid leader hints. Token/ring catalog placements validate their partition coverage and remain non-routing metadata until a later request-routing slice consumes them. | `raftplacement.md` #3046 first slice and token-partition catalog slice | `TestValidateResolvesCollectionLevelPlacements`, `TestValidateRejectsInvalidCatalog`, `TestValidateFeatureFloorFailsClosed`, `TestResolveFailsClosedForUnplacedCollection`, `TestValidateResolvesTokenPartitionPlacements`, `TestResolveCollectionTokenFailsClosedForCollectionPlacement` | implemented |
