...
highest-level node at local ordinal zero. Count and conservative byte
preflights run before row and pack allocation.

The generation's M1 manifest owns exact asset refs, lengths, CRCs, SHA-256
digests, home/overlap memberships, and bounded overlap accounting. A reopened
partition searcher re-verifies the asset and source binding, reports
...
foreign-generation packs fail closed.
