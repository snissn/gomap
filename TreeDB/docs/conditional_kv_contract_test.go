...
func TestConditionalKVContractDocsPinNativeRevisionAndTxnGates(t *testing.T) {
	...
	assertContainsAll(t, contracts, "conditional raw KV contracts",
		...
		"command-WAL LSN for command-WAL raw writes and replay, backend commit sequence for backend-only WAL-off raw writes, cached mutation sequence for cached WAL-off writes, and future Raft apply identity",
	...
	assertContainsAll(t, writePath, "conditional raw KV write path",
		...
		"Backend-only WAL-off raw writes use the backend commit sequence that publishes their root as the mutation revision",
