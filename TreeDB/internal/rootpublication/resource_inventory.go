...
var canonicalReachabilityRequirements = []struct {
	...
	{ReachabilityLegacyVectorSnapshot, ResourceLegacyVectorSnapshot, "explicit-legacy-exclusion", StableProducerLegacyExcluded},
...
var stableResourceInventory = []StableResourceInventoryRow{
	...
	{ReachabilityLegacyVectorSnapshot, ResourceLegacyVectorSnapshot, "TreeDB/collections/vector_index_persist.go compatibility API only", "no root/catalog identity; epoch files are addressed by the explicit compatibility loader", "per-file SHA-256 and size + manifest digest for compatibility validation only", "compatibility snapshot epoch directory and manifest rename", "not registerable by #3677", "TreeDB/collections/vector_index_persist.go explicit load validation", "TreeDB/collections/vector_index_persist.go old-epoch cleanup", "explicit-legacy-exclusion"},
