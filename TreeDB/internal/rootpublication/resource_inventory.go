...
var stableResourceInventory = []StableResourceInventoryRow{
	...
	{ReachabilityDictionaryGeneration, ResourceDictionary, "TreeDB/internal/dictdb/resource_capture.go; TreeDB/caching/db.go", "stable dictdb index generation + exact manager-owned value-log segment + dictionary ID", "dictionary digest + full captured index frontier + exact value-log record end", "dictdb index namespace proof + installed value-log namespace", "CaptureDictionaryResources merged before stable outer-leaf writer mutation", "TreeDB/internal/valuelog/reader.go; TreeDB/internal/valuelog/dict_codec_cache.go", "stable index vacuum fence + value-log identity pin; downstream pin-aware dictionary GC", "authoritative-transitive"},
