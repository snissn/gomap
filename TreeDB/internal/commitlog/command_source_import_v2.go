...
func EncodeCollectionSourceImportPayloadV2(input CollectionSourceImportPayloadV2) ([]byte,error) {
	if len(input.Metadata)==0 || len(input.Metadata)>MaxSourceImportMetadataBytesV2 || input.Inserted.LegacyProjection { return nil,ErrCorrupt }
	rows,err:=EncodeCollectionTypedBatchPayload(input.Inserted)
	if err!=nil{return nil,err}
	if len(rows)>MaxSourceImportPayloadBytesV2-4-len(input.Metadata){return nil,ErrRecordTooLarge}
...
func sourceImportSectionsV2(raw []byte)(metadata,rows []byte,err error){
	if len(raw)<4 || len(raw)>MaxSourceImportPayloadBytesV2{return nil,nil,ErrCorrupt}
	n:=uint64(binary.LittleEndian.Uint32(raw))
	if n==0 || n>MaxSourceImportMetadataBytesV2 || n>uint64(len(raw)-4){return nil,nil,ErrCorrupt}
...
func validateCollectionSourceImportPayloadV2(raw []byte) error {
	...
	if err:=validateCollectionTypedBatchPayload(rows);err!=nil{return err}
	if rows[collectionTypedBatchFlagsOffset]!=0{return ErrCorrupt}
	return nil
}
...
func DecodeCollectionSourceImportPayloadV2(raw []byte)(CollectionSourceImportPayloadV2,error){
	...
	if err!=nil{return CollectionSourceImportPayloadV2{},err}
	inserted,err:=DecodeCollectionTypedBatchPayload(rows)
	if err!=nil{return CollectionSourceImportPayloadV2{},err}
	if inserted.LegacyProjection{return CollectionSourceImportPayloadV2{},ErrCorrupt}
