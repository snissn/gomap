...
func (w *Writer) AppendBatch(records []Record) error {
	...
	// Fast path: when segment compression is disabled, avoid building a
	// contiguous payload buffer. We compute the CRC incrementally and write the
	// payload parts directly to the buffered writer, eliminating the extra
	// scratch copy.
	if !w.compress || w.enc == nil || total < defaultCompressMinLen {
		length := uint32(total)
		if length > segmentLenMask {
			return ErrRecordTooLarge
		}

		var batchHdr [batchHeaderSize]byte
		batchHdr[0] = Version
		binary.LittleEndian.PutUint32(batchHdr[1:5], uint32(len(records)))

		sum := crc.Checksum(batchHdr[:])
		...
		for i := range records {
			...
			valLen := uint32(len(r.Value))
			recHdr[0] = r.Op
			binary.LittleEndian.PutUint16(recHdr[1:3], keyLen)
			binary.LittleEndian.PutUint32(recHdr[3:7], valLen)
			binary.LittleEndian.PutUint64(recHdr[7:15], r.RID)
			binary.LittleEndian.PutUint64(recHdr[15:23], r.Seq)
			sum = crc.Update(sum, recHdr[:])
			sum = crc.Update(sum, r.Key)
			sum = crc.Update(sum, r.Value)
		}

		var segHdr [segmentHeaderSize]byte
		binary.LittleEndian.PutUint32(segHdr[0:4], length)
		binary.LittleEndian.PutUint32(segHdr[4:8], sum)
		if _, err := w.bw.Write(segHdr[:]); err != nil {
			return err
		}
		if _, err := w.bw.Write(batchHdr[:]); err != nil {
			return err
		}
		for i := range records {
			r := &records[i]
			...
			binary.LittleEndian.PutUint64(recHdr[7:15], r.RID)
			binary.LittleEndian.PutUint64(recHdr[15:23], r.Seq)
			if _, err := w.bw.Write(recHdr[:]); err != nil {
				return err
			}
			if len(r.Key) > 0 {
				if _, err := w.bw.Write(r.Key); err != nil {
					return err
				}
			}
			if len(r.Value) > 0 {
				if _, err := w.bw.Write(r.Value); err != nil {
					return err
				}
			}
		}
		w.size += int64(segmentHeaderSize) + int64(length)
		return nil
	}
