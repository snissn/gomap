...
type BackendDB interface {
	...
	// ReverseIterator(start, end []byte) (merging.Iterator, error) // Not used yet
...
func (db *DB) Iterator(start, end []byte) (merging.Iterator, error) {
	...
	return merging.NewMergingIterator(sources), nil
}
...
func (db *DB) ReverseIterator(start, end []byte) (merging.Iterator, error) {
	// TODO: Implement Reverse Merging Iterator.
	// For now fallback to backend (unsafe) or error.
	return nil, fmt.Errorf("cachingdb: reverse iterator not implemented")
}
