...
func sourceShardMapPureV2(input SourceShardMapV2) sourcepartition.SourceShardMapV2 {
	...
	for i, shard := range input.Shards {
		out.Shards[i] = sourcepartition.SourceShardV2{ShardID: shard.ShardID, GroupID: string(shard.GroupID), Start: shard.Start, End: shard.End}
	}
	return out
}
func sourceShardMapFromPureV2(input sourcepartition.SourceShardMapV2) SourceShardMapV2 {
	...
	for i, shard := range input.Shards {
		out.Shards[i] = SourceShardV2{ShardID: shard.ShardID, GroupID: raftcluster.GroupID(shard.GroupID), Start: shard.Start, End: shard.End}
	}
	return out
}

func (c ResolvedCatalogV1) validateSourceGroupsV2(input SourceShardMapV2) error {
	if len(input.Shards) == 0 || len(input.Shards) > MaxSourceShardsV2 {
		return fmt.Errorf("%w: shard count", ErrInvalidSourceShardMapV2)
	}
	if err := validateCollectionRef(input.Collection); err != nil {
		return errors.Join(ErrInvalidSourceShardMapV2, err)
	}
	for _, shard := range input.Shards {
		if _, ok := c.groups[shard.GroupID]; !ok {
			return errors.Join(ErrInvalidSourceShardMapV2, ErrUnknownGroup, fmt.Errorf("source shard %q group %q is absent from catalog", shard.ShardID, shard.GroupID))
		}
	}
	return nil
}
...
func (c ResolvedCatalogV1) CanonicalSourceShardMapV2(input SourceShardMapV2) (SourceShardMapV2, error) {
	if err := c.validateSourceGroupsV2(input); err != nil {
		return SourceShardMapV2{}, err
	}
...
func (m ResolvedSourceShardMapV2) Collection() CollectionRefV1 {
	ref := m.source.Collection()
	return CollectionRefV1{Database: ref.Database, Catalog: ref.Catalog, Collection: ref.Collection}
}
...
func (m ResolvedSourceShardMapV2) ValidateImportIDs(shardID string, ids [][]byte) error {
	return m.source.ValidateImportIDs(shardID, ids)
}
...
func (m ResolvedSourceShardMapV2) SourceMapV2() sourcepartition.ResolvedSourceShardMapV2 {
	return m.source
}
