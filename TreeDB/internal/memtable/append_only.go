...
func getAppendOnlyValuesFromPool(length int, pool *sync.Pool, maxCap int) []string {
	...
	if maxCap <= 0 {
		maxCap = appendOnlyValuePoolMaxCap
	}
	if length > maxCap {
		return make([]string, length)
	}
	...
	if v := pool.Get(); v != nil {
		if values, ok := v.([]string); ok && cap(values) >= length {
			if cap(values) <= appendOnlyMaxReuseEntries(length) {
				out := values[:length]
				clear(out)
				return out
			}
...
func getAppendOnlyPtrPayloadsFromPool(length int, pool *sync.Pool, maxCap int) []appendOnlyPointerPayload {
	...
	if maxCap <= 0 {
		maxCap = appendOnlyPtrPayloadPoolMaxCap
	}
	if length > maxCap {
...
func appendOnlyOrderedIteratorInlineKeys(entries []appendOnlyEntry) []appendOnlyIteratorInlineKey {
	inlineCount := 0
	for i := range entries {
		if appendOnlyEntryInlineKeyLen(&entries[i]) != 0 {
			inlineCount++
		}
	}
	if inlineCount == 0 {
	...
	for i := range entries {
		inlineLen := appendOnlyEntryInlineKeyLen(&entries[i])
		if inlineLen == 0 {
			continue
		}
		inlineKeys = append(inlineKeys, appendOnlyIteratorInlineKey{
			idx:    i,
			length: uint8(inlineLen),
		})
		copy(inlineKeys[len(inlineKeys)-1].key[:], entries[i].inlineKey[:inlineLen])
	}
	return inlineKeys
}
