...
func TestSourceShardMapCanonicalCodecAndPriorDigestV2(t *testing.T) {
	...
	if m.Digest != want { t.Fatalf("changed prior V2 digest: %s", m.Digest) }
	raw, err := EncodeSourceShardMapV2(m)
	if err != nil { t.Fatal(err) }
	resolved, err := DecodeSourceShardMapV2(raw)
	if err != nil || resolved.Digest() != want { t.Fatalf("canonical map decode: %v", err) }
	copy := resolved.Copy()
	copy.Shards[0].GroupID = "changed"
	owner, err := resolved.ResolveDocumentID([]byte("a"))
	if err != nil || owner.GroupID != "group-a" { t.Fatalf("copy changed lookup: %+v %v", owner, err) }
	if err := resolved.ValidateImportIDs("source-a", [][]byte{[]byte("document-123")}); !errors.Is(err, ErrInvalidSourceShardMapV2) { t.Fatalf("nonowner import: %v", err) }
	if err := resolved.ValidateImportIDs("source-a", [][]byte{[]byte("a"), []byte("a")}); !errors.Is(err, ErrInvalidSourceShardMapV2) { t.Fatalf("duplicate exact ID: %v", err) }
	for _, changed := range [][]byte{append(bytes.Clone(raw), '\n'), append(bytes.Clone(raw), raw...), bytes.Replace(raw, []byte(`"Epoch":7`), []byte(`"Epoch":7,"Epoch":7`), 1), bytes.Replace(raw, []byte(`"Epoch":7`), []byte(`"Epoch":7,"unknown":1`), 1), bytes.Replace(raw, []byte(`"Epoch":7`), []byte(`"Epoch":8`), 1)} {
		if _, err := DecodeSourceShardMapV2(changed); !errors.Is(err, ErrInvalidSourceShardMapV2) { t.Fatalf("noncanonical or changed map accepted: %v", err) }
	}
}
...
func TestSourceShardMapPureCoverageAndBoundsV2(t *testing.T) {
	for _, change := range []func(*SourceShardMapV2){
		...
		m := sourceMapFixtureV2(t)
		change(&m)
		if _, err := CanonicalSourceShardMapV2(m); !errors.Is(err, ErrInvalidSourceShardMapV2) { t.Fatalf("invalid complete map accepted: %+v %v", m, err) }
	}
	if _, err := DecodeSourceShardMapV2(make([]byte, MaxSourceShardMapBytesV2+1)); !errors.Is(err, ErrInvalidSourceShardMapV2) { t.Fatalf("encoded cap: %v", err) }
	if _, err := (ResolvedSourceShardMapV2{}).ResolveDocumentID([]byte("a")); !errors.Is(err, ErrInvalidSourceShardMapV2) { t.Fatalf("zero lookup: %v", err) }
}
