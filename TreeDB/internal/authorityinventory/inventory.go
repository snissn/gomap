...
func buildRows() []Row {
	...
	add(active("dictionary.GlobalID", "value-log dictionary dependency", "transitive immutable dictionary closure", "value-log compression encoder", "stable dictdb index generation plus exact manager-owned value-log segment and dictionary ID", "dictionary definition digest plus exact index and value-log frontiers", "dictdb index namespace proof and installed value-log namespace", "dictdb CaptureDictionaryResources merged before stable outer-leaf writer mutation", "dictionary lookup and definition decode", "online index vacuum fence plus value-log identity pin; retain forever until pin-aware dictionary GC exists"))
