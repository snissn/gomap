...
func (f *File) readViaMmapView(ptr page.ValuePtr, verifyCRC bool) ([]byte, error, bool) {
	...
	if fFlags&FrameFlagCompressed != 0 {
		if val, _, err, hit := f.groupedFrameCacheReadTo(start, verifyCRC, k, subIndex, nil); hit {
			if err != nil {
				return nil, err, true
			}
...
func (f *File) readViaMmapViewTo(ptr page.ValuePtr, verifyCRC bool, dst []byte) ([]byte, bool, error, bool) {
	...
	if fFlags&FrameFlagCompressed != 0 {
		if val, usedDst, err, hit := f.groupedFrameCacheReadTo(start, verifyCRC, k, subIndex, dst); hit {
			if err != nil {
				return nil, false, err, true
			}
...
func (f *File) readViaMmapAppend(ptr page.ValuePtr, verifyCRC bool, dst []byte) ([]byte, error, bool) {
	...
	if cachedVal, _, err, hit := f.groupedFrameCacheReadTo(start, verifyCRC, k, subIndex, nil); hit {
		if err != nil {
			return nil, err, true
		}
		oldLen := len(dst)
