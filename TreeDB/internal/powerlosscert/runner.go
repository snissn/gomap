...
func observedWitnessState(recovery recoveryTraceArtifact) WitnessState {
	...
	return WitnessState{
		...
		ExternalFrontiers: fmt.Sprintf("stable_image_tree_sha256=%s manifest_entries=%s",
			recovery.InputTreeSHA256, stat("treedb.durable_root.manifest.entries")),
		NamespaceGeneration: fmt.Sprintf("stable_fingerprint=%s", recovery.StableFingerprint),
