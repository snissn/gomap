...
func TestPromoteLeafGenerationPackSegmentsEmitsRename(t *testing.T) {
	leafDir := t.TempDir()
	stagingDir := filepath.Join(leafDir, ".leaf-pack-copy-test", leafVLogDirName)
	if err := os.MkdirAll(stagingDir, 0o700); err != nil {
		t.Fatalf("MkdirAll(staging): %v", err)
	}
	oldPath := filepath.Join(stagingDir, "value-l255-000001.log")
	if err := os.WriteFile(oldPath, []byte("leaf"), 0o600); err != nil {
		t.Fatalf("WriteFile(staged): %v", err)
	}
	destination := filepath.Join(leafDir, filepath.Base(oldPath))
	...
	promoted, err := promoteLeafGenerationPackSegments([]rewriteCreatedSegment{{path: oldPath, fileID: 1}}, leafDir)
	if err != nil {
		t.Fatalf("promoteLeafGenerationPackSegments: %v", err)
	}
	if len(promoted) != 1 || promoted[0].path != destination {
		t.Fatalf("promoted = %#v, want destination %q", promoted, destination)
	}
	want := []durabilitycut.Event{{
		Resource:  durabilitycut.ResourceOuterLeaf,
		Root:      leafDir,
