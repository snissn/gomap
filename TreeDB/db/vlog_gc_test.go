...
func TestValueLogGC_EmptySet_NoValueLogSegments(t *testing.T) {
	...
	if stats.SegmentsTotal != 0 || stats.SegmentsEligible != 0 || stats.SegmentsDeleted != 0 || stats.BytesTotal != 0 || stats.BytesEligible != 0 || stats.BytesDeleted != 0 || len(stats.EligibleFileIDs) != 0 {
...
func TestValueLogGC_DryRunReportsEligibleFileIDs(t *testing.T) {
	dir := t.TempDir()
	...
	stats, err := db.ValueLogGC(context.Background(), ValueLogGCOptions{DryRun: true})
	if err != nil {
		t.Fatalf("ValueLogGC dry-run: %v", err)
	}
	if got, want := stats.EligibleFileIDs, []uint32{id1}; !reflect.DeepEqual(got, want) {
		t.Fatalf("EligibleFileIDs=%v want=%v", got, want)
	}
	if _, err := os.Stat(path1); err != nil {
		t.Fatalf("dry-run removed eligible segment: %v", err)
	}
	if _, err := os.Stat(path2); err != nil {
		t.Fatalf("dry-run removed referenced segment: %v", err)
	}
}

func TestValueLogGC_SourceFileIDs_TargetsRequestedEligibleSegments(t *testing.T) {
	dir := t.TempDir()
	...
	makeSegment := func(lane, seq uint32, recordID uint64, payload []byte) (string, uint32, page.ValuePtr) {
		path := filepath.Join(walDir, fmt.Sprintf("value-l%d-%06d.log", lane, seq))
		id, err := valuelog.EncodeFileID(lane, seq)
		if err != nil {
			t.Fatalf("fileid%d_%d: %v", lane, seq, err)
		}
		w, err := valuelog.NewWriter(path, id)
		if err != nil {
			t.Fatalf("writer%d_%d: %v", lane, seq, err)
		}
		ptr, err := w.Append(0, nil, recordID, payload)
		if err != nil {
			t.Fatalf("append%d_%d: %v", lane, seq, err)
		}
		if err := w.Close(); err != nil {
			t.Fatalf("close%d_%d: %v", lane, seq, err)
		}
		return path, id, ptr
	}

	path1, id1, _ := makeSegment(0, 1, 1, bytes.Repeat([]byte("value-1|"), 128))
	path2, _, ptr2 := makeSegment(0, 2, 2, bytes.Repeat([]byte("value-2|"), 64))
	path3, id3, _ := makeSegment(1, 1, 3, bytes.Repeat([]byte("value-3|"), 96))
	path4, _, ptr4 := makeSegment(1, 2, 4, bytes.Repeat([]byte("value-4|"), 80))
	...
	if err := ptrBatch.SetPointer([]byte("k4"), ptr4); err != nil {
	...
	stats, err := db.ValueLogGC(context.Background(), ValueLogGCOptions{SourceFileIDs: []uint32{id1, id3}})
	if err != nil {
		t.Fatalf("ValueLogGC: %v", err)
	}
	gotIDs := append([]uint32(nil), stats.EligibleFileIDs...)
	wantIDs := []uint32{id1, id3}
	slices.Sort(gotIDs)
	slices.Sort(wantIDs)
	if !reflect.DeepEqual(gotIDs, wantIDs) {
		t.Fatalf("EligibleFileIDs=%v want=%v", gotIDs, wantIDs)
	}

	if _, err := os.Stat(path1); err == nil || !os.IsNotExist(err) {
		t.Fatalf("expected targeted eligible segment1 removed, err=%v", err)
	}
	if _, err := os.Stat(path3); err == nil || !os.IsNotExist(err) {
		t.Fatalf("expected targeted eligible segment3 removed, err=%v", err)
	}
	if _, err := os.Stat(path2); err != nil {
		t.Fatalf("expected referenced segment2 kept, err=%v", err)
	}
	if _, err := os.Stat(path4); err != nil {
		t.Fatalf("expected referenced segment4 kept, err=%v", err)
	}
}
