...
func TestValueLogRewriteOnline_V1LeafLogRoute_PreservesNestedManifestChunkSegments(t *testing.T) {
	dir := t.TempDir()
	...
	chunkA := bytes.Repeat([]byte("rewriteA|"), 64)
	chunkAPtr, err := chunkAWriter.Append(0, nil, 40_001, chunkA)
	if err != nil {
	...
	chunkB := bytes.Repeat([]byte("rewriteB|"), 64)
	chunkBPtr, err := chunkBWriter.Append(0, nil, 40_002, chunkB)
	if err != nil {
	...
	manifestPtr, err := manifestWriter.Append(0, nil, 40_003, manifestPayload)
	if err != nil {
	...
	key := []byte("k-route-manifest-nested")
	...
	outerPtr, err := outerWriter.Append(0, nil, 40_004, outerPayload)
	if err != nil {
		_ = outerWriter.Close()
		t.Fatalf("append outer: %v", err)
	}
	...
	if _, err := os.Stat(chunkAPath); err != nil {
		t.Fatalf("expected nested chunkA segment to remain after online rewrite: %v", err)
	}
	if _, err := os.Stat(chunkBPath); err != nil {
		t.Fatalf("expected nested chunkB segment to remain after online rewrite: %v", err)
	}
	if _, err := os.Stat(manifestPath); err != nil {
		t.Fatalf("expected nested manifest segment to remain after online rewrite: %v", err)
	}
}
