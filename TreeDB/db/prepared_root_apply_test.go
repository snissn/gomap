...
func TestOrderedRootDeltaBatchGroupPreparedRootMetadataRecordsInstall(t *testing.T) {
	...
	if got := installGuardStatUint(t, stats, "treedb.publish.ordered_root_delta_group.prepared_root.installed_total"); got != 1 {
		t.Fatalf("prepared installed=%d want 1", got)
	}
