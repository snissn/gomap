...
func vacuumIndexOffline(opts Options, fail vacuumFailpoint) error {
	...
	sysIter := tree.New(d.Pager(), newValueReader(state.ValueLogSet, false), state.SystemRootPageID).
	...
	userIter := tree.New(d.Pager(), newValueReader(state.ValueLogSet, false), state.RootPageID).
		IteratorWithOptions(nil, nil, tree.IteratorOptions{Mode: tree.IteratorModePointerProjection})
