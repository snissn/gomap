...
func TestColumnManifestMutationDeltaProductionRepeatedPublishReopen(t *testing.T) {
	...
	for i := 0; i < 8; i++ {
		...
		if work.PhysicalRootShares == 0 {
			t.Fatalf("publish %d did not share retained physical roots: %+v", i, work)
		}
		if work.PhysicalHandleCopies != 0 {
			t.Fatalf("publish %d copied %d physical handles: %+v", i, work.PhysicalHandleCopies, work)
		}
