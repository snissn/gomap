...
func TestVectorPartitionLocalDefaultMaterializationVariantV1(t *testing.T) {
	if got, want := vectorPartitionLocalDefaultGraphVariantV1, VectorPartitionLocalGraphVariantAuxiliaryNavigationM20EfConstruction256V1; got != want {
	...
	if buildDef.M != 20 || buildDef.EfConstruction != 256 || !hasAuxiliaryNavigation {
		t.Fatalf("default materialization definition=%+v auxiliary=%t", buildDef, hasAuxiliaryNavigation)
	}
	var membership [sha256.Size]byte
	membership[0] = 1
	if got, want := vectorPartitionLocalGraphVariantMembershipDigestV1(membership, vectorPartitionLocalDefaultGraphVariantV1), vectorPartitionLocalGraphVariantMembershipDigestV1(membership, VectorPartitionLocalGraphVariantAuxiliaryNavigationM20EfConstruction256V1); got != want {
		t.Fatalf("default materialization membership identity=%x want M20/eFC256 domain=%x", got, want)
	}
	...
	if got, m18 := vectorPartitionLocalGraphVariantMembershipDigestV1(membership, vectorPartitionLocalDefaultGraphVariantV1), vectorPartitionLocalGraphVariantMembershipDigestV1(membership, VectorPartitionLocalGraphVariantAuxiliaryNavigationM18EfConstruction256V1); got == m18 {
		t.Fatal("default materialization retained the M18 membership identity")
	}
}
...
func TestVectorPartitionOfflineAuxiliaryConstructionVariantsV1(t *testing.T) {
	...
	if got, want := canonicalDigest, vectorPartitionLocalGraphVariantMembershipDigestV1(membershipDigest, VectorPartitionLocalGraphVariantAuxiliaryNavigationM20EfConstruction256V1); got != want {
		t.Fatalf("default materializer membership digest=%x want M20/eFC256 domain=%x", got, want)
	}
	...
	if err != nil || canonicalPack.Header.M != 20 || canonicalPack.Header.EfConstruction != 256 || !canonicalPack.Header.HasAuxiliaryNavigation {
	...
	for _, test := range []struct {
		...
		defer resources.Release()
		if test.variant == VectorPartitionLocalGraphVariantAuxiliaryNavigationM20EfConstruction256V1 {
			if assets[0].MembershipDigest != canonical[0].MembershipDigest {
				t.Fatalf("variant=%s did not retain default M20/eFC256 membership digest", test.variant)
			}
		} else if assets[0].MembershipDigest == canonical[0].MembershipDigest {
			t.Fatalf("variant=%s retained default M20/eFC256 membership digest", test.variant)
		}
