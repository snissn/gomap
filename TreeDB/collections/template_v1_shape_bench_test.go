...
func BenchmarkTemplateV1PrepareHomogeneousWide(b *testing.B) {
	...
	benchmarkTemplateV1PrepareDocuments(b, docs, fieldCount, 1)
}
...
func BenchmarkTemplateV1PrepareHeterogeneousShapes(b *testing.B) {
	...
	benchmarkTemplateV1PrepareDocuments(b, docs, fieldCount, shapeCount)
}

func benchmarkTemplateV1PrepareDocuments(b *testing.B, docs [][]byte, fieldCount, shapeCount int) {
	b.Helper()
	prepared, records, resolver, err := prepareTemplateV1InsertDocuments(docs, nil)
	if err != nil {
	...
	b.ResetTimer()
	for i := 0; i < b.N; i++ {
		prepared, _, _, err := prepareTemplateV1InsertDocuments(docs, nil)
		if err != nil {
