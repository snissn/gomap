...
func columnVectorGraphScalarU8AlphaAssetID(q QuantizedVectorIndexDefinition) string {
	...
	if cfgHash := scalarU8CalibrationConfigHashForAssetID(q); cfgHash != 0 {
		return fmt.Sprintf("quantized/%s/scalar_u8/%016x/alpha", q.Name, cfgHash)
	}
	return ""
}
...
func prepareColumnVectorGraphQuantizedAsset(assetRootDir, collection string, base ColumnStoreConfig, def VectorIndexDefinition, graph columnVectorGraphManifestSnapshot, q QuantizedVectorIndexDefinition, generation, partID uint64, rows []columnVectorGraphAssetRow) ([]columnVectorGraphPreparedQuantizedAsset, error) {
	...
	prepared, err := appendColumnVectorGraphQuantizedPreparedAsset(assetRootDir, sourceCfg, q, columnVectorIndexStateAssetRoleQuantizedCodes, columnVectorGraphQuantizedAssetID(q), generation, partID, len(rows), payload)
	if err != nil {
		return nil, err
	}
	...
	if q.Codec == QuantizedVectorCodecScalarU8 && !scalarU8CalibrationIsLegacy(q) {
		...
		alphaPayload, alphaCfg, err := prepareColumnVectorGraphScalarU8AlphaPayload(collection, base, def, q, alphaPartID, metadata)
		if err != nil {
			return nil, err
		}
		alphaPrepared, err := appendColumnVectorGraphQuantizedPreparedAsset(assetRootDir, alphaCfg, q, columnVectorIndexStateAssetRoleQuantizedAlpha, columnVectorGraphScalarU8AlphaAssetID(q), generation, alphaPartID, len(metadata.Alphas), alphaPayload)
		if err != nil {
			return nil, err
		}
...
func columnVectorGraphScalarU8AlphaColumnStoreConfig(collection string, base ColumnStoreConfig, def VectorIndexDefinition, q QuantizedVectorIndexDefinition) (ColumnStoreConfig, error) {
	...
	cfgHash := scalarU8CalibrationConfigHashForAssetID(q)
	if cfgHash == 0 {
...
func buildColumnVectorGraphScalarU8CodesWithAlpha(def VectorIndexDefinition, rows []columnVectorGraphAssetRow, metadata columnVectorGraphScalarU8AlphaMetadata) ([]byte, error) {
	...
	if len(metadata.Alphas) != 0 && len(metadata.Alphas) != len(metadata.RowCounts) {
