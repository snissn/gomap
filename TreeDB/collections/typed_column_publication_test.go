...
func TestTypedColumnPublicationReadCacheNoLeakDuringScan(t *testing.T) {
	...
	if cache.PartLoads != 1 || cache.TypedPartDecodes != 1 || cache.CacheMisses != 1 || cache.CacheHits != rows-1 || len(cache.Rows) != 1 {
		t.Fatalf("cache counters loads=%d decodes=%d hits=%d misses=%d cached_generations=%d want one load/decode/miss/generation and %d hits", cache.PartLoads, cache.TypedPartDecodes, cache.CacheHits, cache.CacheMisses, len(cache.Rows), rows-1)
	}
