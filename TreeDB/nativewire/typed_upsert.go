...
func decodeTypedUpsert(raw, idsRaw, residualRaw []byte, limits iwire.Limits) (string, documentservice.TypedDocumentsRequest, error) {
	return decodeTypedDocuments(raw, idsRaw, residualRaw, limits, false)
}

func decodeTypedDocuments(raw, idsRaw, residualRaw []byte, limits iwire.Limits, allowEmpty bool) (string, documentservice.TypedDocumentsRequest, error) {
	...
	if err != nil || rows < 0 || rows > limits.MaxByteVectorItems || (rows == 0 && !allowEmpty) {
		return "", req, denseDecodeError(err, "invalid row count")
	}
	dims, err := readDenseInt(raw, &off, "dimensions")
	if err != nil || (rows == 0 && dims != 0) || (rows > 0 && (dims <= 0 || dims > 65536 || rows > (len(raw)-off)/4/dims)) {
	...
	if rows == 0 {
		...
		if err != nil || count != 0 || off != len(raw) {
			return "", req, denseDecodeError(err, "invalid empty typed carrier")
		}
...
func (s *Server) handleTypedSourceReplace(ctx context.Context, sections []iwire.Section) ([]iwire.Section, error) {
	...
	if err != nil || deadline <= 0 {
		return nil, denseDecodeError(err, "positive typed source replacement deadline required")
	}
	if ctx == nil {
	...
	var payload [4][]byte
	for i, id := range []iwire.SectionID{iwire.SectionTypedUpsertRequest, iwire.SectionDocumentIDs, iwire.SectionDocuments, iwire.SectionSourceDeleteIDs} {
		data, ok, err := singletonSection(sections, id)
		if err != nil || !ok {
			return nil, denseDecodeError(err, "missing typed source replacement section")
		}
		payload[i] = data
	}
	index, live, err := decodeTypedDocuments(payload[0], payload[1], payload[2], s.limits, true)
	if err != nil {
		return nil, err
	}
	deleteIDs, err := iwire.DecodeByteVectorItemsInto(nil, payload[3], s.limits)
	if err != nil {
		return nil, err
	}
	if len(deleteIDs) > s.limits.MaxByteVectorItems-len(live.IDs) {
		return nil, protocolError(iwire.ErrResourceExhausted, "typed source replacement ID count exceeds limit")
	}
	out, err := s.documentService.ReplaceTypedSourceByID(ctx, index, documentservice.TypedSourceReplacementRequest{DeleteIDs: deleteIDs, TypedDocumentsRequest: live})
	if err != nil {
		return nil, err
	}
	meta := binary.AppendUvarint(nil, out.Index.Generation)
	meta = binary.AppendUvarint(meta, uint64(out.DeletedCount))
	meta = binary.AppendUvarint(meta, uint64(out.InsertedCount))
	return []iwire.Section{{ID: iwire.SectionSourceReplaceResponse, Bytes: meta}}, nil
}
