...
func TestPeerSecurityNodeConnectionsCrossProtocolsV1(t *testing.T) {
	...
	if len(clients) != 4 {
		t.Fatalf("native shared client+server limit: got %d pairs", len(clients))
	}
	...
	if len(sockets) == 0 || len(sockets) >= 4 {
		t.Fatalf("cross-protocol admission allowed %d shard pairs", len(sockets))
	}
	if _, err := runtime.client.Status(ctx, config.NodeID); err != nil {
		t.Fatalf("reserved control capacity starved by native/shard sockets: %v", err)
	}
	stats := transport.ResourceStatsV1()
	if stats.Current[peerConnectionsV1] > 24 || stats.Peak[peerConnectionsV1] > 24 || stats.Rejected[peerConnectionsV1] == 0 || stats.ReadBytes == 0 || stats.WrittenBytes == 0 {
		t.Fatalf("node admission evidence: %+v", stats)
	}
	if err := transport.Close(); err != nil {
		t.Fatal(err)
	}
	stats = transport.ResourceStatsV1()
	if stats.Current[peerConnectionsV1] != 0 || !stats.Closed {
		t.Fatalf("node close leaked charged sockets: %+v", stats)
	}
	if _, err := transport.DialNativeContextV1(ctx, nativeListener.Addr().String(), config.NodeID); err == nil {
		t.Fatal("closed node transport reopened a socket")
	}
}
...
func TestPeerSecurityInvalidNodeBudgetRefusesBeforeStoresV1(t *testing.T) {
	...
	for _, limits := range []PeerNodeLimitsV1{
		...
		if transport, err := NewPeerTransportV1(config); err == nil {
			transport.Close()
			t.Fatalf("accepted invalid/insufficient node budget: %+v", limits)
		}
	}
}
