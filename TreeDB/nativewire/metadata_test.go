...
func TestMetadataCommandsPreserveVectorIndexes(t *testing.T) {
	...
	meta, err := client.CreateCollection(ctx, collections.CollectionMeta{Name: "docs"})
	if err != nil {
		t.Fatalf("CreateCollection: %v", err)
	}
	...
	updated, err := col.CreateVectorIndex(collections.VectorIndexDefinition{
		Name:           "embedding",
		Field:          "embedding",
	...
	if got.SchemaGeneration == 0 {
		t.Fatalf("created vector index schema generation=%d want non-zero", got.SchemaGeneration)
	}
