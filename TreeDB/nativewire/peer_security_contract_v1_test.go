...
func TestPeerSecurityIdentityAndGroupV1(t *testing.T) {
	...
	for name, credentials := range map[string]*PeerCredentialsV1{
		...
		t.Run(name, func(t *testing.T) {
			...
			if err != nil || !roots.AppendCertsFromPEM(raw) { t.Fatalf("roots: %v", err) }
			...
			if err == nil { t.Fatal("unauthorized certificate reached HTTP application response") }
		})
	}
...
func TestPeerSecurityRotationReopenAndDowngradeRefusalV1(t *testing.T) {
	...
	defer cancel()
	if _, err := oldClient.Status(ctx, config.NodeID); err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	oldClient.Close()
	if _, err := oldClient.Status(ctx, config.NodeID); err == nil { t.Fatal("retired trust material rejoined after rotation") }
...
func TestPeerCredentialDeadlineCannotOutliveCertificateV1(t *testing.T) {
	...
	if err := guard.SetReadDeadline(time.Time{}); err != nil { t.Fatal(err) }
	var one [1]byte
