...
func normalizePeerNodeLimitsV1(value PeerNodeLimitsV1) (PeerNodeLimitsV1, error) {
	...
	for i, field := range fields {
		...
		if *field < 1 || *field > maxima[i] {
			return value, fmt.Errorf("%w: invalid node resource limit %d", raftcluster.ErrInvalidConfig, i)
		}
	}
...
func newPeerNodeAdmissionV1(config FixedPeerTCPConfigV1) (*peerNodeAdmissionV1, error) {
	...
	for kind, remaining := range a.sharedLimits {
		if remaining <= 0 {
			return nil, fmt.Errorf("%w: node limit %d cannot cover declared group reserves", raftcluster.ErrInvalidConfig, kind)
		}
	}
...
func (a *peerNodeAdmissionV1) acquire(scope string, kind int, amount int64) (peerResourceLeaseV1, error) {
	...
	s := a.scopes[scope]
	if a.stats.Closed {
		return lease, net.ErrClosed
	}
	if s == nil || kind < 0 || kind >= peerResourceKindsV1 || amount < 0 {
		return lease, raftcluster.ErrInvalidConfig
	}
	if amount == 0 {
		return lease, nil
	}
	if amount > s.limits[kind] {
		a.stats.Rejected[kind]++
		return lease, raftcluster.ErrAdmissionUnavailable
	}
...
func (a *peerNodeAdmissionV1) close() error {
	...
	for _, listener := range listeners {
		if e := listener.Close(); !errors.Is(e, net.ErrClosed) {
			err = errors.Join(err, e)
		}
	}
	for _, conn := range conns {
		if e := conn.Close(); !errors.Is(e, net.ErrClosed) {
			err = errors.Join(err, e)
		}
	}
	return err
