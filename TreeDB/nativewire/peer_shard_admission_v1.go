...
func readPeerShardFrameV1(reader io.Reader, maxFrame uint32, admission *peerNodeAdmissionV1, scope string) (vectorPartitionShardSearchTCPFrameV1, peerWorkLeaseV1, error) {
	...
	if admission == nil { frame, err := readVectorPartitionShardSearchTCPFrameV1(reader, maxFrame); return frame, work, err }
	var prefix [4]byte
	if _, err := io.ReadFull(reader, prefix[:]); err != nil { return vectorPartitionShardSearchTCPFrameV1{}, work, err }
	size := binary.BigEndian.Uint32(prefix[:])
	if size == 0 || size > maxFrame { return vectorPartitionShardSearchTCPFrameV1{}, work, raftcluster.ErrAdmissionUnavailable }
	...
	if err != nil { return vectorPartitionShardSearchTCPFrameV1{}, work, err }
	raw := make([]byte, size)
	if _, err := io.ReadFull(reader, raw); err != nil { work.release(); return vectorPartitionShardSearchTCPFrameV1{}, work, err }
	frame, err := decodeVectorPartitionShardSearchTCPFrameBodyWithResponseBoundsV1(raw, 32, 256)
	if err != nil { work.release() }
...
func peerShardResponseFrameV1(request VectorPartitionShardSearchRequestV1, configured uint32) (uint32, error) {
	...
	if len(request.PartitionIDs) == 0 || len(request.PartitionIDs) > limits.MaxPartitions || request.TopK < 1 || request.TopK > limits.MaxTopK { return 0, raftcluster.ErrRouteTargetUnsupported }
