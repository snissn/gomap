...
func TestPeerSecurityStalledSnapshotPreservesOtherGroupV1(t *testing.T) {
	...
	go func() {
		...
		done <- sender.InstallSnapshot(hraft.ServerID(config.NodeID), hraft.ServerAddress(config.RaftListen[group.ID]), &request, &response, reader)
	}()
	...
	fixedPeerWaitV1(t, ctx, func() bool {
		state, err := node.data[group.ID].provider.RuntimeStatusV1(ctx)
		return err == nil && state.Term == request.Term
	})
	...
	if stats := node.PeerTransportV1().ResourceStatsV1(); stats.Current[peerSnapshotsV1] != 1 || stats.Rejected[peerSnapshotsV1] == 0 {
		t.Fatalf("stalled snapshot admission: %+v", stats)
	}
	...
	if _, err := node.client.PublishCatalog(ctx, config.NodeID, command); err != nil {
		t.Fatalf("stalled data snapshot starved real catalog commit: %v", err)
	}
	writer.Close()
	select {
	case <-done:
