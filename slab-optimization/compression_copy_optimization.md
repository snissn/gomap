...
### 2026-01-21 (Branch1 Step 5 – direct-append EncodeAll into appendBuf)
- Implemented direct-append EncodeAll path for dict-on frames (uses appendBuf as encDst; avoids encoded→appendBuf copy for small/medium frames).
- Re-profiled below cutoff (ultra, 1KiB):
  - mode4 dict-on: ops/s ~1,036k, MB/s ~1,012; memmove ~33% flat; memtable/skiplist still dominant; writer share small.
  - mode3 dict-on: ops/s ~519k, MB/s ~507; memmove ~45% flat; appendValueLog ~24% cum; zstd EncodeAll ~21% cum.
- Result: mode4 improved vs earlier run; mode3 still dominated by memmove + EncodeAll. Next step: verify memmove attribution vs previous snapshot and decide if additional writer-side copy elimination is worth it or if memtable path is now dominant.
