...
- **2026-01-21**: PR100 bumps `github.com/snissn/compress` to `7ad45194ecdc` (dict reset optimization) and fixes a
  vlogprof flake where dict-on profiles could fail to apply a dict after warmup under higher throughput.
...
- **2026-01-21**: PR102 (reverted) attempted to speed up `largePtrMap.SetString` by switching from `sync.RWMutex` to
  `sync.Mutex` and preallocating map capacity; pprof showed mapassign dominates and throughput did not improve reliably.
