...
retains the full historical campaign. The command atomically rewrites its JSON
and Markdown reports after each completed phase and guardrail failure.
`selected_phases`, `completed_phases`, and `complete` distinguish surviving
partial evidence from a completed run. A failed guardrail is always incomplete
evidence: strict mode returns the guardrail error after persisting phase data,
while `-allow-guardrail-failures` may continue eligible later diagnostic phases
