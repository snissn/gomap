...
Use the bounded retrieval phase when load and query evidence is required without
running concurrent, maintenance, or backfill probes:

```sh
RUN_DIR=/tmp/gomap_text_hybrid_retrieval_1m_$(date +%Y%m%d_%H%M%S) \
RUN_1M=true RUN_SMOKE=false PHASES=retrieval RETRIEVAL_REPETITIONS=3 \
ONE_M_ROWS=1000000 ONE_M_QUERIES=25 ONE_M_BATCH_SIZE=16384 \
ONE_M_CANDIDATE_LIMIT=65536 \
scripts/bench_text_hybrid_scale.sh
```

`PHASES=retrieval` selects load, queries, and reopen. The default
`PHASES=all` retains the full historical campaign. Repetitions greater than one
are accepted only for the exact retrieval selector; the wrapper rejects
nonpositive repetition counts and repeated full campaigns before writing plans.
...
- `selected_phases`, `completed_phases`, and `complete` — atomic phase progress;
  an incomplete report is partial evidence, not a completed qualification.
