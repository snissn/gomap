...
- when `PAIR_ALIGN_STOP_HEIGHT_FROM_FIRST=1`, the second leg also forces a tighter stop poll:
  - `PAIR_ALIGN_STOP_POLL_INTERVAL_SECONDS` (default `0.05`)
  - keep this small when catch-up is fast; with `0.2s` polling we observed second-leg stop overshoot of `12` and `27` blocks in real pairs
...
- `analyze_vlog_maintenance_capacity.py --json` now also emits `write_path_summary` from the same live stats payload when available:
  ...
  - use those when the selector appears to prefer dict but sampled `write_mode.dict` bytes stay at zero; they tell whether the outer-leaf dict path was actually selected, whether it later downgraded before encode, and whether the class-specific dict was only published very late in the run
