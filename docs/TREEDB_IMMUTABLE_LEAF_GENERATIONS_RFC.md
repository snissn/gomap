...
- repeatable planner profiling still lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh), but the harness now also carries a persisted-index reopen benchmark (`BenchmarkLeafGenerationPlanPersistedIndexReopen_SavedHome`) plus an isolated live-stats benchmark (`BenchmarkLeafGenerationLiveStatsPersistedIndex_SavedHome`) on a copied work fixture; the latest raw output is captured in [leafgen_live_stats_bench_20260414.txt](/tmp/leafgen_live_stats_bench_20260414.txt)
- on `/home/mikers/.application-db-engine-matrix-splitouterleaf-20260414090917/treedb/application.db`, the persisted-index reopen benchmark now lands around `111.8` to `115.2 ms/op`, `15.93 MiB/op`, and only `180` to `190 allocs/op`; the isolated live-stats scan lands around `113.7` to `123.5 ms/op`, `15.92 MiB/op`, and only `136` to `149 allocs/op`
- this is a materially different planner profile than the earlier sealed-sidecar-only state: the previous `~31k allocs/op` came from current writable leaf files still falling back to per-record header reads, while the new in-memory writable-file offset/length index removes that churn and collapses alloc objects by about two orders of magnitude on the saved home
- the saved-home page-stat benchmark now reports `9,560` total pager pages, `9,560` unique pager pages, `0` duplicate pager pages across user+system roots, and `978,081` leaf refs; that justifies keeping the runtime path on the safe shared-root dedup walker rather than carrying a riskier acyclic shortcut for little measured gain
- current profile evidence from [cpu_top.txt](/tmp/leafgen_plan_profile_index_20260414/cpu_top.txt), [alloc_objects_top.txt](/tmp/leafgen_plan_profile_index_20260414/alloc_objects_top.txt), and the latest saved-home bench output now points to tree walk, pager reads, and decode/decompression as the remaining cold-path cost; sealed-file rediscovery and writable-file header churn are no longer the dominant allocation problem
