...
- latest `TREEDB_PROFILE=wal_on_fast` dwell over the same 60-second saved-home run now reports `sample_count=5`, `max_rss_kb=212,192`, `max_hwm_kb=212,200`, final `scheduler_state=idle`, `leaf_pack_attempts=2`, `leaf_pack_runs=0`, and `leaf_pack_skips=2`; that closes the liveness gap from the earlier hard-disabled scheduler, but it also shows a new cost signal: `wal_on_fast` now pays a materially higher RSS peak than `fast` while still correctly declining pack work on this dense home
- the dwell harness now respects `TREEDB_ENABLE_LEAF_GENERATION_PACK_MAINTENANCE`, and the first `wal_on_fast` control run with that flag forced to `0` reports `max_rss_kb=76,200`, final `scheduler_state=idle`, and zero leaf-pack attempts/runs/skips; on this home the roughly `136 MiB` delta between the enabled and disabled runs is therefore attributable to the current leaf-pack attempt path rather than to merely keeping the scheduler loop alive
- repeatable planner profiling now lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh); on the same saved Celestia home, one `LeafGenerationPlan` pass measures about `747 ms/op`, `134,136,216 B/op`, and `986,254 allocs/op`
- the first saved-home planner profile isolates the immediate optimization targets: CPU is dominated by `valueLogRecordLengthForRewriteInSet`/`readValueLogRecordLengthFromHeader` preads, `alloc_objects` is almost entirely header-read churn, and `alloc_space` is mostly the scan visitor plus the `seen map[LeafLogPtr]struct{}` footprint inside `scanLeafGenerationLiveStats`; that means the next leaf-pack planner sprint should focus on avoiding per-page header reads on the common path and proving whether the `seen` dedup map is actually required
