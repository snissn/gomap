...
- repeatable planner profiling still lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh), but the harness now also carries a persisted-index reopen benchmark (`BenchmarkLeafGenerationPlanPersistedIndexReopen_SavedHome`) plus an isolated live-stats benchmark (`BenchmarkLeafGenerationLiveStatsPersistedIndex_SavedHome`) on a copied work fixture; the latest raw output is captured in [leafgen_live_stats_bench_20260414_phase2.txt](/tmp/leafgen_live_stats_bench_20260414_phase2.txt)
- on `/home/mikers/.application-db-engine-matrix-splitouterleaf-20260414090917/treedb/application.db`, the persisted-index reopen benchmark now lands around `67.7` to `69.3 ms/op`, `16.48 MiB/op`, and `261` to `272 allocs/op`; the isolated live-stats scan lands around `67.9` to `69.3 ms/op`, `16.47 MiB/op`, and `218` to `228 allocs/op`
- the mechanism for that improvement is now explicit in code: `scanLeafGenerationLiveStats` preloads per-file record-length index state once per scan and uses slice-backed generation/file totals during the `LeafRef` walk, instead of re-resolving generation/index state and updating hash maps on every live leaf page
- this leaves the planner in a materially different place than the earlier sealed-sidecar-only state: first the writable-file in-memory offset/length index removed the old `~31k allocs/op` header-read churn, and then the per-scan file-state preload plus slice accumulators cut cold saved-home wall time by roughly another `40%` while keeping the runtime path on the safe shared-root dedup walker
...
- latest isolated profile evidence from [cpu_top.txt](/tmp/leafgen_live_stats_only_profile_phase2_prfNwn/cpu_top.txt), [alloc_space_top.txt](/tmp/leafgen_live_stats_only_profile_phase2_prfNwn/alloc_space_top.txt), and [alloc_objects_top.txt](/tmp/leafgen_live_stats_only_profile_phase2_prfNwn/alloc_objects_top.txt) now shows the remaining cold-path cost concentrated in `leafGenerationRecordLengthIndex.lookup`, the shared-root pager walk, checksum verification, and cold `.lenidx` file loads; planner bookkeeping itself is no longer the dominant issue
