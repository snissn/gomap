...
- latest cached dwell result on `/home/mikers/.application-db-engine-matrix-splitouterleaf-20260414090917/treedb/application.db` with `TREEDB_PROFILE=fast` over a 60-second dwell: `sample_count=5`, `max_rss_kb=93,900`, `max_hwm_kb=93,908`, final `scheduler_state=idle`, `leaf_pack_attempts=2`, `leaf_pack_runs=0`, and `leaf_pack_skips=2`; the forced plan still remained roughly `4,585,857` reclaimable bytes against `2,142,908,926` bytes to copy, so the hook is bounded and explainable on this home but not yet useful
...
- latest `TREEDB_PROFILE=wal_on_fast` dwell over the same 60-second saved-home run now reports `sample_count=5`, `max_rss_kb=99,088`, `max_hwm_kb=99,088`, final `scheduler_state=idle`, `leaf_pack_attempts=2`, `leaf_pack_runs=0`, and `leaf_pack_skips=2`; that closes the liveness gap from the earlier hard-disabled scheduler without the previous planner-memory blow-up, and the branch now holds `fast` and `wal_on_fast` to essentially the same steady-state leaf-pack cost on this dense home
- the dwell harness now respects `TREEDB_ENABLE_LEAF_GENERATION_PACK_MAINTENANCE`, and the first `wal_on_fast` control run with that flag forced to `0` reports `max_rss_kb=76,200`, final `scheduler_state=idle`, and zero leaf-pack attempts/runs/skips; after adding the published-state live-scan cache, the remaining steady-state delta on this home is now about `17 MiB` in `fast` and about `23 MiB` in `wal_on_fast`, which means the earlier planner-memory blow-up was largely removed and the residual cost of a skipped leaf-pack attempt is now much smaller
- repeatable planner profiling now lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh); the cold saved-home benchmark now explicitly clears the planner cache before each run and measures about `582 ms/op`, `24,062,888 B/op`, and `978,230 allocs/op`, which keeps the first-pass cost honest instead of accidentally timing only the warmed state
- the same benchmark file now also carries a cached steady-state benchmark: after one cold plan populates the published-state cache, repeated `LeafGenerationPlan` calls on the same saved Celestia home drop to about `5,633 ns/op`, `5,160 B/op`, and `33 allocs/op`; the cache is keyed by published `(commitSeq, rootID, systemRoot)` and is shared by both `LeafGenerationPlan` and whole-generation `LeafGenerationGC` reachability collection
- the saved-home duplicate scan benchmark still reports `978,081` total leaf refs, `978,081` unique leaf refs, and `0` duplicates, which justified removing the planner's per-`LeafLogPtr` seen map and cut the dwell RSS peak by about `53 MiB` in `fast` and about `111 MiB` in `wal_on_fast`; with repeated steady-state rescans now effectively eliminated on an unchanged root, the remaining cold-path optimization target is even cleaner: CPU and allocation objects are still dominated by `valueLogRecordLengthForRewriteInSet`/`readValueLogRecordLengthFromHeader` preads, so the next planner sprint should focus on eliminating per-page header reads on the common path
- the write path already has the missing information: `AppendLeafPage` receives a `ValuePtr` whose `Length` carries the compressed record-length hint, and `LeafLogPtrFromValuePtr` currently discards that hint because `LeafRef` still uses a fixed 64-bit `(marker,file_id,offset)` encoding; the preferred follow-on is therefore not to widen `LeafRef`, but to add sealed-generation record-length metadata keyed by file offset so cold planner / GC scans can avoid one header `pread` per reachable leaf page while keeping pointer identity and internal-page wire format stable
