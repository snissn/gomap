...
- the write path already has the missing information: `AppendLeafPage` receives a `ValuePtr` whose `Length` carries the compressed record-length hint, and `LeafLogPtrFromValuePtr` currently discards that hint because `LeafRef` still uses a fixed 64-bit `(marker,file_id,offset)` encoding; this branch now exploits that structure for sealed files by building a lazy offset/length index per sealed `leaf_vlog` file, but the remaining cold-path hotspot is still the one-time sealed-file scan itself, not repeated header `pread`s
- current profile evidence from [cpu_top.txt](/tmp/leafgen_plan_profile_index_20260414/cpu_top.txt) and [alloc_objects_top.txt](/tmp/leafgen_plan_profile_index_20260414/alloc_objects_top.txt) is now much cleaner: only about `20 ms` cumulative remains in `readValueLogRecordLengthFromHeader`, while the dominant one-time planner cost moved to `scanLeafGenerationRecordLengthIndex`; the next structural step is therefore persisted sealed-generation length metadata so the first cold plan can avoid even that scan while keeping pointer identity and internal-page wire format stable
