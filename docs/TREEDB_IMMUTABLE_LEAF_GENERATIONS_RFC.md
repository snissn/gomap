...
- repeatable planner profiling now lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh); on the same saved Celestia home, the latest `LeafGenerationPlan` pass measures about `559 ms/op`, `23,484,360 B/op`, and `978,124 allocs/op` after removing the redundant per-`LeafLogPtr` seen map from the planner scan
- the saved-home duplicate scan benchmark now reports `978,081` total leaf refs, `978,081` unique leaf refs, and `0` duplicates, which justified removing the planner's per-`LeafLogPtr` seen map and cut the dwell RSS peak by about `53 MiB` in `fast` and about `111 MiB` in `wal_on_fast`; the remaining optimization target is now even cleaner: CPU and allocation objects are still dominated by `valueLogRecordLengthForRewriteInSet`/`readValueLogRecordLengthFromHeader` preads, so the next planner sprint should focus on eliminating per-page header reads on the common path
