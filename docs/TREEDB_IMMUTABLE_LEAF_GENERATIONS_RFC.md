...
- the same benchmark file still carries a cached steady-state benchmark: after one cold plan populates the published-state cache, repeated `LeafGenerationPlan` calls on the same saved Celestia home stay around `5,633 ns/op`, `5,160 B/op`, and `33 allocs/op`; the cache is keyed by published `(commitSeq, rootID, systemRoot)` and is shared by both `LeafGenerationPlan` and whole-generation `LeafGenerationGC` reachability collection
...
- repeatable planner profiling still lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh), but the harness now also carries a persisted-index reopen benchmark (`BenchmarkLeafGenerationPlanPersistedIndexReopen_SavedHome`) plus an isolated live-stats benchmark (`BenchmarkLeafGenerationLiveStatsPersistedIndex_SavedHome`) on a copied work fixture; the latest raw output is captured in [leafgen_live_stats_bench_20260414_phase3.txt](/tmp/leafgen_live_stats_bench_20260414_phase3.txt)
- on `/home/mikers/.application-db-engine-matrix-splitouterleaf-20260414090917/treedb/application.db`, the persisted-index reopen benchmark now lands around `42.5` to `43.6 ms/op`, `16.48 MiB/op`, and `261` to `282 allocs/op`; the isolated live-stats scan lands around `42.7` to `43.8 ms/op`, `16.47 MiB/op`, and `216` to `232 allocs/op`
- the mechanism for that additional improvement is also explicit in code: after preloading per-file record-length index state once per scan, the hot path now carries a per-file lookup hint into `leafGenerationRecordLengthIndex.lookupWithHint`, which shrinks the search work materially on the saved Celestia home without changing planner semantics
- this leaves the planner in a materially different place than the earlier sealed-sidecar-only state: first the writable-file in-memory offset/length index removed the old `~31k allocs/op` header-read churn, then per-scan file-state preload plus slice accumulators cut cold saved-home wall time by roughly `40%`, and the per-file lookup hint cut another roughly `35%` from the remaining cold live-scan cost while keeping the runtime path on the safe shared-root dedup walker
...
- latest isolated profile evidence from [cpu_top.txt](/tmp/leafgen_live_stats_only_profile_phase3_EuiJnn/cpu_top.txt), [alloc_space_top.txt](/tmp/leafgen_live_stats_only_profile_phase3_EuiJnn/alloc_space_top.txt), and [alloc_objects_top.txt](/tmp/leafgen_live_stats_only_profile_phase3_EuiJnn/alloc_objects_top.txt) now shows the remaining cold-path cost concentrated in hinted index lookup/search, the shared-root pager walk, checksum verification, and cold `.lenidx` file loads; planner bookkeeping itself is no longer the dominant issue
