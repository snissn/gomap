...
- repeatable planner profiling still lives at [TreeDB/db/leaf_generation_plan_bench_test.go](/home/mikers/dev/snissn/gomap/TreeDB/db/leaf_generation_plan_bench_test.go) and [scripts/leafgen_plan_profile.sh](/home/mikers/dev/snissn/gomap/scripts/leafgen_plan_profile.sh), but the harness now also carries a persisted-index reopen benchmark (`BenchmarkLeafGenerationPlanPersistedIndexReopen_SavedHome`) on a copied work fixture; on `/home/mikers/.application-db-engine-matrix-splitouterleaf-20260414090917/treedb/application.db` that measured about `339.4 ms/op`, `16,224,232 B/op`, and `31,408 allocs/op` for the cold fallback path versus `118.8 ms/op`, `16,047,736 B/op`, and `31,308 allocs/op` once persisted `.lenidx` sidecars were present, which is the expected signature of removing sealed-file rediscovery while leaving the tree walk as the main remaining cost
- current profile evidence from [cpu_top.txt](/tmp/leafgen_plan_profile_index_20260414/cpu_top.txt) and [alloc_objects_top.txt](/tmp/leafgen_plan_profile_index_20260414/alloc_objects_top.txt) is still directionally useful for the fallback path: only about `20 ms` cumulative remains in `readValueLogRecordLengthFromHeader`, while the dominant one-time planner cost moved to `scanLeafGenerationRecordLengthIndex`; with persisted sidecars now in place, the next real hotspot to study is the tree walk and per-generation reachability accounting rather than sealed-file length discovery
