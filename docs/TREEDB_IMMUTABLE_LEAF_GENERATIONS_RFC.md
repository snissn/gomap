...
- that next structural step is now landed in this branch: `scanLeafGenerationLiveStats` memoizes immutable pager-page subtree generation totals keyed by page ID, so root churn only rescans new path pages while unchanged subtrees reuse cached reachability totals; `TestLeafGenerationPlan_ReusesCachedSubtreesAcrossRootChanges` now proves the second plan after a small write misses fewer cached pages than the initial full scan, and the latest raw output in [leafgen_live_stats_bench_20260415_phase14.txt](/tmp/leafgen_live_stats_bench_20260415_phase14.txt) lands the verified-page persisted-index path around `13.5` to `16.2 ms/op` and the true warm repeated-scan path around `6.0` to `8.2 us/op`, with the steady-state allocs trimmed to `15 allocs/op`
