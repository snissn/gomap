...
- Default: no cache journal (WAL) (highest throughput; pending writes are lost on crash before flush).
- Optional: enable a per-shard cache journal (WAL) via `hashdb.OpenWithOptions` / `hashdb.OpenWithShardsAndOptions`:
