...
- Keep env and flags identical (journal/WAL, value log, compression, memtable settings).
...
- `TREEDB_TRACE_DISABLE_WAL`: `1/0` (disables the journal for durability)
...
- Hardware / CPU model
- Memtable mode and key options (journal/WAL, value log, thresholds)
