...
- Cached mode persists writes to `Dir/wal/` (journal segments) and flushes them into the backend in the background.
- On open, TreeDB always performs a single coherent recovery path:
  1) backend recovery (meta validation + torn-tail handling)
  2) journal replay into the backend (synced commits)
...
### 2) Journal discovery + replay (always)
...
- Opening backend-only immediately after cached mode is safe: backend open replays journal segments first.
- Opening cached mode immediately after backend-only is safe: there are no journal segments unless cached mode previously ran.
