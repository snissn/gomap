...
func highlightLines(cells []cellComparison) []string {
	...
	if cell := largestDiskCell(cells); cell != nil {
		...
		lines = append(lines, fmt.Sprintf("Largest cell disk: at %d docs / %d indexes / `%s`, TreeDB %s snapshot used %s (%s/doc), TreeDB physical du was %s, MongoDB dbStats dataSize was %s, MongoDB dbStats totalSize was %s, and MongoDB physical du was %s (%s/doc).",
			cell.Key.Documents,
