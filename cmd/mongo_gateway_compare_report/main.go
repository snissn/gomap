...
func renderReport(cfg config, cells []cellComparison, generatedAt time.Time) string {
	...
	b.WriteString("- `concurrent_*_rN` phases are read throughput sweeps over `N` concurrent readers, and are grouped in the Concurrent Read Sweep section when present.\n")
