...
func TestQualificationValidatorRejectsEveryNorthStarGap4329(t *testing.T) {
	tests := []struct {
		...
		{"missing reopen postings route", "explicit hybrid postings route", func(r *report) { r.Reopen.HybridStats.TextPostingsScanned = unscaledDefaultPostingsCeiling }},
...
func validQualificationReport4329() report {
	...
	return report{
		...
		Reopen: &reopenReport{
			Status: "passed", CountOK: true, QueryParityOK: true, BeforeResultsSHA256: "same", AfterResultsSHA256: "same",
			TextStorage: accounting("after_reopen"), StorageBytes: 1, OracleVersion: scaleRelevanceOracleVersion,
			TextPrecisionAtK: 1, HybridPrecisionAtK: 1, QualityOracleOK: true,
			HybridPostingsBudget: requiredHybridMaxPostingsScanned,
			HybridStats: &collections.HybridSearchStats{
				TextCandidatesRequested: requiredHybridCandidateLimit, TextCandidateBudgetEffective: requiredHybridCandidateLimit,
				TextCandidatesReturned: 1, TextPostingsScanned: unscaledDefaultPostingsCeiling + 1,
				VectorCandidatesRequested: requiredHybridCandidateLimit, VectorCandidateBudgetEffective: requiredHybridCandidateLimit,
				VectorCandidatesReturned: 1,
			},
			Resource: resource,
		},
