...
func TestRunBenchmark_RandomReadBatch_WithConfiguredReadWorkers_Smoke(t *testing.T) {
	run, err := runBenchmark(BenchConfig{
		Keys:         2_000,
		ValueSize:    16,
		BatchSize:    128,
	...
	got := run.Results["random_read_batch"]["TreeDB"]
	if math.IsNaN(got) || got <= 0 {
		t.Fatalf("expected random_read_batch > 0 for custom read-workers, got %v", got)
	}
}
...
func TestRunBenchmark_RandomReadParallelAcquireSnapshot_Smoke(t *testing.T) {
	...
	for _, dbName := range []string{"TreeDB", "LevelDB"} {
		got := run.Results["random_read_parallel_acquire_snapshot"][dbName]
		if math.IsNaN(got) || got <= 0 {
			t.Fatalf("expected random_read_parallel_acquire_snapshot > 0 for %s, got %v", dbName, got)
		}
	}
}
