...
- `-treedb-index-outer-leaf-mode` TreeDB: outer-leaf mode selector (`v1|v2_blockptr|v2_fenceptr`)
...
- `-blockprofile`, `-mutexprofile` write global profiling artifacts to files and also emit per-test contention delta profiles in the same directory (`block_<test>_<db>.pprof`, `mutex_<test>_<db>.pprof`)
...
- `block_<test>_<db>.pprof`
- `mutex_<test>_<db>.pprof`
