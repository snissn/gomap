...
func m8QualificationRetainedAttributionV1(root string, report m8ProductionReportV1, truth [][]m8CanonicalResultV1, transcript m8ProductionMeasurementTranscriptV1) (err error) {
	...
	if report.Config.RouterRepresentation != nil {
		if err := harness.enableRouterRepresentationV1(context.Background(), *report.Config.RouterRepresentation); err != nil {
			return err
		}
	}
	...
	for rowIndex, row := range report.Rows {
		...
		if err := m8RouterRepresentationEvidenceSelectionV1(report.Config, row); err != nil {
			return err
		}
