...
func testM8QualificationReportV1(t *testing.T, head string, fixture fixtureManifest, descriptor m3VariantDescriptorV1, p4QPS float64, fullLadder bool) m8ProductionReportV1 {
	...
	for i := range report.RouterSessions.AfterWarmup {
		report.RouterSessions.AfterWarmup[i].Identity.ReadySetDigest = descriptor.ReadySetDigest
		report.RouterSessions.AfterWarmup[i].Identity.RouterModelDigest = descriptor.RouterModelDigest
	}
	for i := range report.RouterSessions.AfterMeasured {
		report.RouterSessions.AfterMeasured[i].Identity.ReadySetDigest = descriptor.ReadySetDigest
		report.RouterSessions.AfterMeasured[i].Identity.RouterModelDigest = descriptor.RouterModelDigest
	}
