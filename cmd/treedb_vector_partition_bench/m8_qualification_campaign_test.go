...
func TestM8QualificationRetainedVariantV1(t *testing.T) {
	...
	t.Run("replays_retained_attribution", func(t *testing.T) {
		...
		report.Config.RouterCandidates = 1024 // Includes upper-layer and repeated score calls.
		report.Config.RouterWidth, report.Config.RouterBeam = assets.routerWidth, assets.routerBeam
		...
		oracles, err := m8MembershipOracleRecallCacheV1(truth, primaryHomes, finalMemberships, assets.manifest, 1)
		if err != nil {
			_ = harness.Close()
			_ = assets.Close()
			t.Fatal(err)
		}
		cell, err := m8BuildAttributionV1(context.Background(), assets, primaryHomes, finalMemberships, queries, truth, oracles, 1, 64, report.Config.TopK, report.Config.RouterCandidates, make([][]m8CanonicalResultV1, len(queries)), harness)
...
func testM8QualificationReportV1(t *testing.T, head string, fixture fixtureManifest, descriptor m3VariantDescriptorV1, p4QPS float64) m8ProductionReportV1 {
	...
	report.Config.RouterWidth, report.Config.RouterBeam = int(descriptor.RouterRepresentatives), int(descriptor.RouterRepresentatives)
...
func testM8QualificationCommandV1(report m8ProductionReportV1, out string) []string {
	...
	args := []string{
		...
		"-router-width", strconv.Itoa(cfg.RouterWidth), "-router-beam", strconv.Itoa(cfg.RouterBeam),
