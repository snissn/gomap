...
type m8ProductionConfigEvidenceV1 struct {
	...
	RouterRepresentation    *collections.VectorPartitionRouterRepresentationOptionsV1 `json:"router_representation,omitempty"`
	RouterEffort            *collections.VectorPartitionRouterEffortOptionsV1         `json:"router_effort,omitempty"`
...
type m8ProductionAttributionV1 struct {
	...
	RouterRepresentation *m8RouterRepresentationEvidenceV1 `json:"router_representation_diagnostics,omitempty"`
	RouterEffort         *m8RouterEffortEvidenceV1         `json:"router_effort_diagnostics,omitempty"`
	Contract             string                            `json:"contract"`
...
func runM8ProductionSingleVariantV1(cfg config, fixture fixtureManifest, vectors, queries [][]float64, stdout io.Writer) (runErr error) {
	...
	if cfg.m8RouterEffort.Mode != "" {
		if err := collections.ValidateVectorPartitionRouterEffortOptionsV1(cfg.m8RouterEffort, int(assets.status.Representatives)); err != nil {
			return err
		}
	}
	...
	if len(measuredCells) > 0 {
		...
		if cfg.m8RouterEffort.Mode != "" {
			if err := attributionHarness.enableRouterEffortV1(cfg.m8RouterEffort); err != nil {
				return err
			}
		}
		if cfg.m8RouterRepresentation.Arm != "" {
			if err := attributionHarness.enableRouterRepresentationV1(context.Background(), cfg.m8RouterRepresentation); err != nil {
				return err
			}
		}
...
type m8AttributionHarnessV1 struct {
	...
	effort         *m8RouterEffortCacheV1
	representation *m8RouterRepresentationCacheV1
...
func m8BuildAttributionV1(ctx context.Context, assets *m8ProductionMultiGroupAssetsV1, primaryHomes map[string]uint32, finalMemberships map[string][]uint32, queries [][]float64, truth [][]m8CanonicalResultV1, membershipOracles []m8MembershipOracleRecallV1, probes, efSearch, topK, approximateCandidates int, exhaustive [][]m8CanonicalResultV1, harness *m8AttributionHarnessV1) (m8AttributionCellV1, error) {
	...
	effort, effortErr := harness.routerEffortEvidenceV1(ctx, queries, truth, probes, approximateCandidates)
	if effortErr != nil {
		return cell, effortErr
	}
	cell.Evidence.RouterEffort = effort
	...
	representation, representationErr := harness.routerRepresentationEvidenceV1(ctx, queries, truth, probes, approximateCandidates)
	if representationErr != nil {
		return cell, representationErr
	}
	cell.Evidence.RouterRepresentation = representation
