...
func TestM8RouterRepresentationActualProducerKeepsMeasuredBoundary(t *testing.T) {
	...
	args := []string{
		...
		"-m8-router-representation-arm", "centroid_geometry", "-m8-router-representation-width", "2",
		"-m8-router-representation-max-build-work", "200000000", "-m8-router-representation-max-build-bytes", "67108864",
	}
	...
	if len(report.Rows) != 4 || report.Config.RouterRepresentation == nil || report.Config.RouterEffort == nil {
	...
	for i, row := range report.Rows {
		if row.Samples != fixture.Queries || row.Attribution.RouterRepresentation == nil || len(row.Attribution.RouterRepresentation.Queries) != fixture.Queries {
			t.Fatal("lost representation population")
		}
		...
		for _, q := range row.Attribution.RouterRepresentation.Queries {
			if len(q.Arms) != 3 || len(q.TruthDomains) == 0 {
				t.Fatal("lost geometry arms/explanations")
			}
		}
	}
	// A changed diagnostic cannot be smuggled into its measured transcript.
	report.Rows[0].Attribution.RouterRepresentation.Info.SourceChecksum++
