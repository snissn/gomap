...
func m0MaterializeVariantV1(raw string) (collections.VectorPartitionLocalGraphVariantV1, int, int, error) {
	if variant := collections.VectorPartitionLocalGraphVariantV1(raw); variant == collections.VectorPartitionLocalGraphVariantCanonicalHNSWM18EfConstruction256V1 {
		return variant, 18, 256, nil
	}
...
func runM0MaterializeMembershipV1(args []string, stdout io.Writer) (err error) {
	...
	fs.StringVar(&variantRaw, "variant", string(collections.VectorPartitionLocalGraphVariantCanonicalHNSWM18EfConstruction256V1), "production partition-local HNSW variant")
