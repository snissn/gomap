...
type localHNSWAttributionSourceEvidenceV1 struct {
	...
	Domains               uint32                `json:"logical_domains"`
...
func runLocalHNSWAttributionV1(args []string, stdout io.Writer) (runErr error) {
	...
	report := localHNSWAttributionReportV1{
		...
		TopK:       10, EFSearch: 128, ProbeCounts: []int{min(2, int(source.manifest.DomainCount)), int(source.manifest.DomainCount)}, NativeBuild: nativeBuild, OverlayBuild: overlayBuild,
